<?php
	...
	function updateAmount($srr_no) {
		global $mydb;
		list($total) = $mydb->getArray("select ifnull(sum(qty*cost-disc),0) from srr_details where srr_no = '$srr_no' and branch = '$_SESSION[branchid]';");
	...
	switch($_REQUEST['mod']) {
		...
		case "addItem":
			list($isE) = $mydb->getArray("select count(*) from srr_details where trace_no = '$_POST[trace_no]' and item_code = '$_POST[item]' and lot_no = '$_POST[lotno]' and branch = '$_SESSION[branchid]';");
			if($isE > 0) {
				$s = "update ignore srr_details set qty = qty + ".$mydb->formatDigit($_POST['qty']).", amount = amount + ".$mydb->formatDigit($_POST['amount']).", lot_no = '$_POST[lotno]', expiry = '$_POST[expiry]' where trace_no = '$_POST[trace_no]' and item_code = '$_POST[item]' and branch = '$_SESSION[branchid]';";
			} else {
				$s = "insert ignore into srr_details (branch,srr_no,item_code,description,qty,unit,cost,lot_no,expiry,amount,trace_no) values ('$_SESSION[branchid]','$_POST[srr_no]','$_POST[item]','".$mydb->escapeString(htmlentities($_POST['description']))."','".$mydb->formatDigit($_POST['qty'])."','$_POST[unit]','".$mydb->formatDigit($_POST['cost'])."','$_POST[lotno]','".$mydb->formatDate($_POST['expiry'])."','".$mydb->formatDigit($_POST['amount'])."','$_POST[trace_no]');";
			}
		...
		case "applyLineDiscount":
			$d = $mydb->getArray("select * from srr_details where line_id = '$_POST[lid]';");
			
            // $discount = ROUND(($d['amount']) * ($_POST['discPercent']/100),2);
            // $adue = $d['amount'] - $discount;  
			$adue = ROUND($d['amount']) - ($_POST['discPercent']);    
           
			$mydb->dbquery("update ignore srr_details set disc = '$_POST[discPercent]',  amount = '$adue' where line_id = '$_POST[lid]';");
		...
		case "retrieveLine":
			echo json_encode($mydb->getArray("select *,format(cost,2) as ucost, lot_no, expiry, format(amount,2) as amt from srr_details where line_id = '$_POST[lid]';"));
		break;

		case "updateItem":
			$mydb->dbquery("update srr_details set item_code = '$_POST[item]', description = '".$mydb->escapeString(htmlentities($_POST['description']))."', qty = '".$mydb->formatDigit($_POST['qty'])."', unit = '$_POST[unit]',cost = '".$mydb->formatDigit($_POST['cost'])."', lot_no = '$_POST[lotno]', expiry = '".$mydb->formatDate($_POST['expiry'])."', amount = '".$mydb->formatDigit($_POST['amount'])."' where line_id = '$_POST[lid]';");
			updateAmount($_POST['srr_no']);
		break;
		...
		case "finalizeSRR":
			...
			$iquery = $mydb->dbquery("SELECT a.branch, a.srr_no AS doc_no, a.srr_date AS doc_date, received_from AS customer, b.item_code, b.unit, b.qty, b.expiry FROM srr_header a INNER JOIN srr_details b ON a.srr_no = b.srr_no AND a.branch = b.branch WHERE a.srr_no = '$_POST[srr_no]' AND a.branch = '$_SESSION[branchid]';");
			while($ibook = $iquery->fetch_array(MYSQLI_BOTH)) {
				$mydb->dbquery("INSERT IGNORE INTO ibook (doc_no,doc_date,doc_type,doc_branch,cname,item_code,uom,expiry,inbound,posted_by,posted_on) VALUES ('$ibook[doc_no]','$ibook[doc_date]','SRR','$ibook[branch]','".$mydb->escapeString($ibook['customer'])."','$ibook[item_code]','$ibook[unit]','$ibook[expiry]','$ibook[qty]','$_SESSION[userid]',now());");
			}
		...
		case "retrieve":
			...
			$srrd = $mydb->dbquery("SELECT line_id as id, description, item_code, unit, qty, cost, lot_no, date_format(expiry,'%m/%d/%Y') as expiry, amount FROM srr_details where trace_no = '$_REQUEST[trace_no]';");
