    ...
    switch($_REQUEST['mod']) {
        ...
        case "saveHeader":
            if($_POST['soa_no'] == '') {
                ...
                $con->dbquery("INSERT IGNORE INTO soa_header (trace_no,soa_no,stub_no,branch,soa_date,customer_code,customer_name,customer_address,remarks,terms,referred_by,revenue_center,po_no,po_date,vat,created_by,created_on) VALUES ('$_POST[trace_no]','$soano','$_POST[stub_no]','$_SESSION[branchid]','".$con->formatDate($_POST['soa_date'])."','$_POST[cid]','".$con->escapeString(htmlentities($_POST['cname']))."','".$con->escapeString(htmlentities($_POST['caddr']))."','".$con->escapeString(htmlentities($_POST['remarks']))."','$_POST[terms]','".$con->escapeString(htmlentities($_POST['referred_by']))."','$_POST[revcenter]','$_POST[po_no]','".$con->formatDate($_POST['po_date'])."','$_POST[vat]','$_SESSION[userid]',now());");
            } else {
                $con->dbquery("UPDATE IGNORE soa_header set stub_no = '$_POST[stub_no]', soa_date='".$con->formatDate($_POST['soa_date'])."',customer_code='$_POST[cid]',customer_name='".$con->escapeString(htmlentities($_POST['cname']))."',customer_address='".$con->escapeString(htmlentities($_POST['caddr']))."',remarks='".$con->escapeString(htmlentities($_POST['remarks']))."',terms='$_POST[terms]',referred_by='".$con->escapeString(htmlentities($_POST['referred_by']))."',revenue_center = '$_POST[revcenter]', po_no = '$_POST[po_no]', po_date = '".$con->formatDate($_POST['po_date'])."', vat = '$_POST[vat]', updated_by='$_SESSION[userid]',updated_on=now() where soa_no = '$_POST[soa_no]' and branch = '$_SESSION[branchid]';");
               $soano = $_POST['soa_no'];
            }
        ...
        case "retrieve":
            $data = array();
			$srrd = $con->dbquery("SELECT line_id as lid, if(source='SO',concat('SO-',LPAD(so_no,6,0)),so_no) AS sono, DATE_FORMAT(so_date,'%m/%d/%Y') AS sdate, if(pid!='',CONCAT('(',LPAD(pid,6,'0'),') ',pname),pname) AS pname, `code`, description,unit,FORMAT(qty,2) AS qty,FORMAT(unit_price,2) AS price, FORMAT(amount,2) AS amount, `source`, `pid`, so_no as xso FROM soa_details WHERE trace_no = '$_REQUEST[trace_no]';");
        ...
        case "uploadSO":
            ...
            foreach($arr as $so_no) {
                $t = $con->dbquery("SELECT a.so_no, a.so_date, a.patient_id AS pid, a.patient_name AS pname, b.code, b.description, b.qty, b.unit, b.unit_price-b.discount as unit_price, b.amount_due as amount FROM so_header a LEFT JOIN so_details b ON a.so_no = b.so_no AND a.branch = b.branch WHERE a.so_no = '$so_no' and a.branch = '$_SESSION[branchid]';");
                while($s = $t->fetch_array()) {
                    $con->dbquery("INSERT IGNORE INTO soa_details (soa_no,branch,source,so_no,so_date,pid,pname,`code`,`description`,qty,unit,unit_price,amount,trace_no) VALUES ('$_POST[soa_no]','$_SESSION[branchid]','SO','$s[so_no]','$s[so_date]','$s[pid]','".$con->escapeString($s['pname'])."','$s[code]','".$con->escapeString($s['description'])."','$s[qty]','$s[unit]','$s[unit_price]','$s[amount]','$_POST[trace_no]');");
                }
            }
        ...
        case "browseCSO":
            ...
            $q = "SELECT b.line_id AS lid, LPAD(a.cso_no,6,0) AS sono, DATE_FORMAT(cso_date,'%m/%d/%Y') AS sdate, b.pid, b.pname AS patient_name, a.customer_name AS charge_to, b.description AS `service`, b.amount, a.remarks FROM pccmobile.cso_header a LEFT JOIN pccmobile.cso_details b ON a.cso_no = b.cso_no AND a.branch = b.branch LEFT JOIN contact_info c ON a.customer_code = c.file_id WHERE a.branch = '$_SESSION[branchid]' AND customer_code = '$_POST[cid]' AND b.billed != 'Y' AND b.cso_no NOT IN (SELECT DISTINCT pid FROM soa_details WHERE soa_no = '$_POST[soa_no]' AND branch = '$_SESSION[branchid]') AND `status` = 'Finalized' $s ORDER BY a.cso_no ASC, b.pname ASC;";
        ...
        case "uploadCSO":
            ...
            $arr = $_POST['so'];
            foreach($arr as $lid) {
                $csoRow = $con->getArray("select * from pccmobile.cso_details where line_id = '$lid';");
                list($unit) = $con->getArray("select unit from services_master where `code` = '$csoRow[code]';");
                list($csodate) = $con->getArray("select cso_date from pccmobile.cso_header where cso_no = '$csoRow[cso_no]';");

                $con->dbquery("INSERT IGNORE INTO soa_details (soa_no,branch,source,so_no,so_date,pid,pname,`code`,description,unit,qty,unit_price,amount,trace_no) VALUES ('$_POST[soa_no]','1','CSO','$csoRow[cso_no]','$csodate','$csoRow[pid]','$csoRow[pname]','$csoRow[code]','$csoRow[description]','$unit','$csoRow[branch]','$csoRow[unit_price]','$csoRow[amount]','$_POST[trace_no]');");
                $con->dbquery("UPDATE IGNORE pccmobile.cso_details SET billed = 'Y', soa_no = '$_POST[soa_no]' WHERE line_id = '$lid';");
        ...
        case "uploadPharmaSO":
            ...
            foreach($arr as $lid) {
                ...
                list($csodate) = $con->getArray("select so_date,line_id from pharma_so_header where so_no = '$csoRow[so_no]';");
                list($pid,$pname) = $con->getArray("select pid, patient_name from pharma_so_header where so_no = '$csoRow[so_no]';");

                $con->dbquery("INSERT IGNORE INTO soa_details (soa_no,branch,source,so_no,so_date,pid,pname,`code`,description,unit,qty,unit_price,amount,disc,trace_no) VALUES ('$_POST[soa_no]','1','PHARMA','$csoRow[so_no]','$csodate','$pid','$pname','$csoRow[code]','$csoRow[description]','$unit','$csoRow[qty]','$csoRow[unit_price]','$csoRow[amount_due]','$csoRow[discount]','$_POST[trace_no]');");
        ...
        case "addItem": 
            $con->dbquery("INSERT IGNORE INTO soa_details (soa_no,branch,pname,source,so_no,so_date,`code`,`description`,unit,unit_price,qty,amount,trace_no) VALUES ('$_POST[soa_no]','$_SESSION[branchid]','".htmlentities($_POST['pname'])."','OTHERS','$_POST[refno]','".$con->formatDate($_POST['date'])."','$_POST[item]','".$con->escapeString(htmlentities($_POST['description']))."','$_POST[unit]','".$con->formatDigit($_POST['cost'])."','".$con->formatDigit($_POST['qty'])."','".$con->formatDigit($_POST['amount'])."','$_POST[trace_no]');");
             updateAmount($_POST['soa_no']);
        break;
        ...
        case "deleteItem":
            ...
            if($csoRow['source'] == 'CSO') {
                $con->dbquery("UPDATE IGNORE pccmobile.cso_details SET billed = 'N', soa_no = '' where cso_no = '$csoRow[so_no]' and branch = '1' and pid = '$csoRow[pid]' and `code` = '$csoRow[code]';");
            }

            if($csoRow['source'] == 'PHARMA') {
                $con->dbquery("UPDATE pharma_so_details SET billed = 'N' where so_no = '$csoRow[so_no]' and branch = '1' and code = '$csoRow[code]';");
            }
        ...
        case "finalize":
            ...
            /* Update SO Status */
            $sQuery = $con->dbquery("SELECT DISTINCT so_no FROM soa_details WHERE soa_no = '$_POST[soa_no]' AND branch = '$_SESSION[branchid]' and `source` = 'SO';");
        ...
        case "cancel":
            $con->dbquery("update ignore soa_header set `status` = 'Cancelled', updated_by = '$_SESSION[userid]', updated_on = now() where soa_no = '$_POST[soa_no]' and branch = '$_SESSION[branchid]';");
        break;
    }
