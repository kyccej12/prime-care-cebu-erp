...
<script>
	...
	$(document).ready(function() {
		var myTable = $('#itemlist').DataTable({
			...
			"bProcessing": true,
			"responsive": true,
			"sAjaxSource": "data/imgsamples.php?displayPending=<?php echo $_REQUEST['displayPending'] . "&sid=" . $_REQUEST['sid']; ?>",
			"order": [[1, "desc"]],
			"scroller": true,
		...
		$("#printDtf").datepicker(); $("#printDt2").datepicker();
		$('#printCompany').autocomplete({
	...
	function refreshList() {
		...
		var xurl = "data/imgsamples.php?type="+type+"&sid="+Math.random()+"";
	...
	function printByBatch() {
		var dis = $("#printBatch").dialog({
			title: "Print Results By Batch",
			width: 540,
			resizeable: false,
			modal: true,
			buttons: [
				{
					...
					click: function() {
						window.open("print/result.xray.batch.php?dtf="+$("#printDtf").val()+"&dt2="+$("#printDt2").val()+"&company="+$("#printCompany").val()+"&consultant="+$("#printConsultant").val()+"&encoder="+$("#printEncoder").val()+"&sort="+$("#printSort").val()+"&type="+$("#xray_type").val()+"&sid="+Math.random(),"Unclassified Accounts","location=1,status=1,scrollbars=1,width=640,height=720");
					}
				},
				{
					text: "Close",
				...
				<table width="100%" cellspacing="0" cellpadding="0" style="margin-top: 5px; margin-bottom: 5px;">
					<tr>
						...
						<td align=right class="spandix-l">
							Display Records :&nbsp;&nbsp;&nbsp;&nbsp;<select id="displayType" name="displayType" class="gridInput" style="width: 250px; height: 24px; font-size: 11px;">
								<option value="">Today's Requests</option>
								<option value="1">Last 7 Days</option>
								<option value="2">Last 14 Days</option>
								<option value="3">Last 30 Days</option>
								<option value="4">- All Request -</option>
...
<div id="printBatch" name="printBatch" style="display: none;">
	<table width=100% cellpadding=2 cellspacing=0>
		<tr>
			<td width=35% class="spandix-l">Result Date Coverage :</td>
			<td><input type="text" id="printDtf" name="printDtf" class="gridInput"  style="width: 80%;" value="<?php echo date('m/d/Y'); ?>"></td>
		</tr>
		<tr>
			<td width=35% class="spandix-l"></td>
			<td><input type="text" id="printDt2" name="printDt2" class="gridInput" style="width: 80%;" value="<?php echo date('m/d/Y'); ?>"></td>
		</tr>
		<tr>
			<td width=35% class="spandix-l">Consultant :</td>
			<td>
				<select name="printConsultant" id="printConsultant" class="gridInput" style="width: 80%; font-size: 11px;">
					<option value=''>All Consultants</option>
					<?php
						$query = $o->dbquery("select id, fullname from options_doctors order by fullname desc;");
		...
		<tr>
			...
			<td>
				<select name="printEncoder" id="printEncoder" class="gridInput" style="width: 80%; font-size: 11px;">
...
<form name="frmDisplayType" id="frmDisplayType" action="imgsamples.list.php" method="GET">
