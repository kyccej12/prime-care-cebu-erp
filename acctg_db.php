<?php
	...
	list($lm_dtf) = $con->getArray("select date_sub('".$con->formatDate($dtf)."', INTERVAL 1 MONTH);"); 
	list($ly_dtf) = $con->getArray("select date_sub('".$con->formatDate($dtf)."', INTERVAL 1 YEAR);");
	list($lm_dt2) = $con->getArray("select date_sub('".$con->formatDate($dt2)."', INTERVAL 1 MONTH);");
	//list($ly_dt2) = $con->getArray("select date_sub('".$con->formatDate($dt2)."', INTERVAL 1 YEAR);");
	$year = $_GET[year];
	$ly = $year - 1; 
	$ly_dt2 = date("m/d/$ly");
	...
	/* Year Comparison */
	list($ytd) = $con->getArray("select sum(credit-debit) from acctg_gl a left join acctg_accounts b ON a.acct = b.acct_code where doc_date between '$year-01-01' and '".$con->formatDate($dt2)."' AND a.branch = '$_SESSION[branchid]' and b.acct_grp in ('9','10');");
	list($lytd) = $con->getArray("select sum(credit-debit) from acctg_gl a left join acctg_accounts b on a.acct = b.acct_code where doc_date between '$ly-01-01' and '".$con->formatDate($ly_dt2)."' AND a.branch = '$_SESSION[branchid]' and b.acct_grp in ('9','10');");
	$yvariance = ROUND((($ytd - $lytd) / $ytd) * 100);

	/* Month Comparison */
	list($cm) = $con->getArray("select sum(credit-debit) from acctg_gl a left join acctg_accounts b ON a.acct = b.acct_code where doc_date between '".$con->formatDate($dtf)."' and '".$con->formatDate($dt2)."' and b.acct_grp in ('9','10') AND a.branch = '$_SESSION[branchid]';");
	list($lm) = $con->getArray("select sum(credit-debit) from acctg_gl a left join acctg_accounts b ON a.acct = b.acct_code where doc_date between '$lm_dtf' and '$lm_dt2' and b.acct_grp in ('9','10') AND a.branch = '$_SESSION[branchid]';");
	list($ym) = $con->getArray("select sum(credit-debit) from acctg_gl a left join acctg_accounts b ON a.acct = b.acct_code where doc_date between '$ly_dtf' and '".$con->formatDate($ly_dt2)."' and b.acct_grp in ('9','10') AND a.branch = '$_SESSION[branchid]';");

	//echo "select sum(credit-debit) from acctg_gl a left join acctg_accounts b ON a.acct = b.acct_code where doc_date between '$ly_dtf' and '".$con->formatDate($ly_dt2)."' and b.acct_grp in ('9','10') AND a.branch = '$_SESSION[branchid]';";
	...
	/* YTD NET INCOME */
	list($ytd_i) = $con->getArray("SELECT SUM(credit-debit) FROM acctg_gl a left join acctg_accounts b ON a.acct = b.acct_code WHERE doc_date BETWEEN '$_GET[year]-01-01' AND '".$con->formatDate($dt2)."' AND a.branch = '$_SESSION[branchid]' AND b.acct_grp IN ('9','10','11','12','13','14');");
	list($lytd_i) = $con->getArray("SELECT SUM(credit-debit) FROM acctg_gl a left join acctg_accounts b ON a.acct = b.acct_code WHERE doc_date BETWEEN '$ly-01-01' AND '".$con->formatDate($ly_dt2)."' AND a.branch = '$_SESSION[branchid]' AND b.acct_grp IN ('9','10','11','12','13','14');");
					...
					<tr>
						<td width=100%>
							<table width=100% style="border: 1px solid #cdcdcd;">
								...
								<?php
									$z = $con->dbquery("SELECT * FROM (SELECT CONCAT('(',a.acct,') ',b.description), ROUND(SUM(debit-credit),2) AS amt FROM acctg_gl a left join acctg_accounts b ON a.acct = b.acct_code WHERE doc_date BETWEEN '".$con->formatDate($dtf)."' AND '".$con->formatDate($dt2)."' AND b.acct_grp in ('12') GROUP BY a.acct) a ORDER BY amt DESC LIMIT 10;");
									$i = 1;
