<?php
	...
	switch($_POST['mod']) {
		...
		case "addInvoice":
			...
			if($ewt > 0) { $p->dbquery("insert ignore into cv_details (branch,cy,cv_no,ref_no,ref_date,ref_type,acct,acct_desc,credit,acct_branch,cost_center) values ('1','".$p->formatCY($_POST['cv_date'])."','$cv_no','$_POST[ref_no]','".$p->formatDate($_POST['ref_date'])."','SI','$ewtAcct','".$p->getAcctDesc($ewtAcct,1)."','$ewt','1','$_POST[costcenter]');"); }
			if($vat > 0) { $p->dbquery("insert ignore into cv_details (branch,cy,cv_no,ref_no,ref_date,ref_type,acct,acct_desc,debit,acct_branch,cost_center) values ('1','".$p->formatCY($_POST['cv_date'])."','$cv_no','$_POST[ref_no]','".$p->formatDate($_POST['ref_date'])."','SI','30208','".$p->getAcctDesc("30208",1)."','$vat','1','$_POST[costcenter]');"); }
			list($bExist) = $p->getArray("select count(*) from cv_details where cv_no = '$cv_no' and acct = '$_POST[bank]' and branch = '$_SESSION[branchid]';");
			if($_POST['dbAcct'] != '') {
				$p->dbquery("insert ignore into cv_details (branch,cy,cv_no,ref_no,ref_date,ref_type,acct,acct_desc,debit,acct_branch,cost_center) values ('1','".$p->formatCY($_POST['cv_date'])."','$cv_no','$_POST[ref_no]','".$p->formatDate($_POST['ref_date'])."','SI','".$_POST['dbAcct']."','".$p->getAcctDesc($_POST['dbAcct'],1)."','$net','1','$_POST[costcenter]');");
			}
			if($bExist > 0) {
				...
				$p->dbquery("insert ignore into cv_details (branch,cy,cv_no,ref_no,ref_date,ref_type,acct,acct_desc,credit,acct_branch,cost_center) values ('1','".$p->formatCY($_POST['cv_date'])."','$cv_no','$cv_no','".$p->formatDate($_POST['cv_date'])."','CV','$_POST[bank]','".$p->escapeString($p->getAcctDesc($_POST['bank'],1))."','$payable','1','$_POST[costcenter]');");
			}
