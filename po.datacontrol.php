<?php
	...
	switch($_POST['mod']) {
		...
		case "addItem":
			$p->dbquery("INSERT IGNORE INTO po_details (branch,po_no,costcenter,item_code,description,qty,unit,cost,discount,amount,trace_no) VALUES ('$bid','$_POST[po_no]','$_POST[costcenter]','$_POST[item]','".$p->escapeString(htmlentities($_POST['description']))."','".$p->formatDigit($_POST['qty'])."','$_POST[unit]','".$p->formatDigit($_POST['cost'])."','".$p->formatDigit($_POST['discount'])."','".$p->formatDigit($_POST['amount'])."','$_POST[trace_no]');");
			$p->updateHeaderAmt($_POST['po_no'],$bid);
		break;

		case "updateItem":
			$p->dbquery("UPDATE IGNORE po_details set costcenter = '$_POST[costcenter]',item_code = '$_POST[item]',description='".$p->escapeString(htmlentities($_POST['description']))."',qty = '".$p->formatDigit($_POST['qty'])."',unit = '$_POST[unit]',cost = '".$p->formatDigit($_POST['cost'])."', discount = '".$p->formatDigit($_POST['discount'])."', amount = '".$p->formatDigit($_POST['amount'])."' where line_id = '$_POST[lid]';");
			$p->updateHeaderAmt($_POST['po_no'],$bid);
		break;

		case "applyDiscount":
			$p->dbquery("update IGNORE po_details set discount = '".$p->formatDigit($_POST['discount'])."', amount = ROUND(qty * (cost-".$p->formatDigit($_POST['discount'])."),2) where line_id = '$_POST[lid]';");
			$p->updateHeaderAmt($_POST['po_no'],$bid);
		break;
		...
		case "finalizePO":
			$p->dbquery("update IGNORE po_header set status = 'Finalized', updated_by = '$uid', updated_on = now() where po_no ='$_POST[po_no]' and branch = '$bid';");
			$p->updateHeaderAmt($_POST['po_no'],$bid);
		break;

		case "reopenPO":
			$p->dbquery("update IGNORE po_header set status = 'Active', updated_by = '$uid', updated_on = now() where po_no = '$_POST[po_no]' and branch = '$bid';");
		break;

		case "cancel":
			$p->dbquery("update IGNORE po_header set status = 'Cancelled', updated_by = '$uid', updated_on = now() where po_no = '$_POST[po_no]' and branch = '$bid';");
		break;
		...
		case "getTotals":
			list($gross,$net,$discount) = $p->getArray("select sum(ROUND(qty*cost,2)) as gross, sum(ROUND(qty*(cost-discount),2)) as net, sum(ROUND(qty*discount,2)) as total_discount from po_details where po_no = '$_POST[po_no]' AND branch = '$bid';");
			echo json_encode(array("gross"=>number_format($gross,2),"discount"=>number_format($discount,2),"net"=>number_format($net,2)));
		break;

		case "retrieve":
			$data = array();
	
			$srrd = $p->dbquery("SELECT line_id AS id, a.costcenter, b.costcenter AS cc, description, item_code, unit, qty, cost, discount, amount FROM po_details a LEFT JOIN options_costcenter b ON a.costcenter = b.unitcode WHERE trace_no = '$_REQUEST[trace_no]';");
