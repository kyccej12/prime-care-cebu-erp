<?php
	...
	$objPHPExcel->getProperties()->setCreator("Medgruppe Polyclinics & Diagnostic Center, Inc.")
								 ...
								 ->setTitle("Medgruppe Polyclinics & Diagnostic Center, Inc. - Purchases Summary")
								 ->setSubject("Medgruppe Polyclinics & Diagnostic Center, Inc. - Purchases Summary")
								 ->setDescription("Medgruppe Polyclinics & Diagnostic Center, Inc. - Purchases Summary")
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("A5","Summary of Purchases ($_GET[dtf] - $_GET[dt2]");
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("H7","TAX CODE");
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("I7","GROSS");
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("J7","V-A-T");
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("K7","EWT");
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("L7","NET");
	...
	while($data = $query->fetch_array(MYSQLI_BOTH)) {
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(7,$row,$data['ewt_code']);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(8,$row,$data['gross']);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(9,$row,$data['input_vat']);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(10,$row,$data['ewt']);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(11,$row,$data['net']);
		
		
		for($ix=0;$ix<=11;$ix++) { $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($ix,$row)->applyFromArray($contentStyle); }
		for($ixy=8;$ixy<=11;$ixy++) { $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($ixy,$row)->getNumberFormat()->setFormatCode('#,##0.00'); }
		$row++; $grossGT+=$data['gross']; $inputGT+=$data['input_vat']; $ewtGT+=$data['ewt']; $netGT+=$data['net']; $brgy = ""; $ct = ""; $prov = "";
	}
	
	/* TOTAL */
	$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(8,$row,$grossGT);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(9,$row,$inputGT);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(10,$row,$ewtGT);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(11,$row,$netGT);

	for($ixt=8;$ixt<=11;$ixt++) {
