<?php
	...
	$query = $mydb->dbquery("SELECT * FROM (SELECT 'so' AS `type`, a.so_no, DATE_FORMAT(a.so_date,'%m/%d/%Y') AS sodate,'' AS or_no, '' AS ordate, a.customer_code, a.customer_name, a.patient_id, a.patient_name, c.description AS xterms, b.code,b.description, b.amount_due, a.amount AS charge_sales, '0' AS cash_sales, '0' AS cc_sales, '0' AS check_sales, '0' AS ad_payment, a.physician FROM so_header a LEFT JOIN so_details b ON a.trace_no = b.trace_no LEFT JOIN options_terms c ON a.terms = c.terms_id LEFT JOIN services_master d ON b.code = d.code WHERE a.status = 'Finalized' AND a.terms NOT IN ('0','100') AND a.so_date BETWEEN '$dtf' AND '$dt2' $searchString UNION ALL SELECT 'ap' AS `type`, a.so_no, DATE_FORMAT(a.so_date,'%m/%d/%Y') AS sodate,'' AS or_no, '' AS ordate, a.customer_code, a.customer_name, a.patient_id, a.patient_name, c.description AS xterms, b.code,b.description, b.amount_due, '0' AS charge_sales, '0' AS cash_sales, '0' AS cc_sales, '0' AS check_sales, a.amount AS ad_payment, a.physician FROM so_header a LEFT JOIN so_details b ON a.trace_no = b.trace_no LEFT JOIN options_terms c ON a.terms = c.terms_id WHERE a.status = 'Finalized' AND a.terms IN ('100') AND a.so_date BETWEEN '$dtf' AND '$dt2' $searchString UNION ALL SELECT 'or' AS `type`, b.so_no, DATE_FORMAT(b.so_date,'%m/%d/%Y') AS so_date, a.or_no AS or_no, DATE_FORMAT(a.doc_date,'%m/%d/%Y') AS ordate, a.customer_code, a.customer_name, b.pid AS patient_id, b.pname AS patient_name, 'Cash' AS terms, b.code, b.description, b.amount_due, 0 AS charge_sales, a.amount_due AS cash_sales, '0' AS cc_sales, '0' AS check_sales, '0' AS ad_payment, '' as physician FROM or_header a LEFT JOIN or_details b ON a.trace_no = b.trace_no WHERE a.status = 'Finalized' AND a.doc_date BETWEEN '$dtf' AND '$dt2' AND a.cashtype IN ('1') $searchString UNION ALL SELECT 'cc' AS `type`, b.so_no, DATE_FORMAT(b.so_date,'%m/%d/%Y') AS so_date, a.doc_no AS or_no, DATE_FORMAT(a.doc_date,'%m/%d/%Y') AS ordate, a.customer_code, a.customer_name, b.pid AS patient_id, b.pname AS patient_name, 'Cash' AS terms, b.code, b.description, b.amount_due, 0 AS charge_sales, '0' AS cash_sales, a.amount_due AS cc_sales, '0' AS check_sales, '0' AS ad_payment, '' as physician FROM or_header a LEFT JOIN or_details b ON a.trace_no = b.trace_no WHERE a.status = 'Finalized' AND a.doc_date BETWEEN '$dtf' AND '$dt2' AND a.cashtype IN ('2') $searchString UNION ALL SELECT 'ck' AS `type`, b.so_no, DATE_FORMAT(b.so_date,'%m/%d/%Y') AS so_date, a.doc_no AS or_no, DATE_FORMAT(a.doc_date,'%m/%d/%Y') AS ordate, a.customer_code, a.customer_name, b.pid AS patient_id, b.pname AS patient_name, 'Cash' AS terms, b.code, b.description, b.amount_due, 0 AS charge_sales, '0' AS cash_sales, '0' AS cc_sales, a.amount_due AS check_sales, '0' AS ad_payment,'' as physician FROM or_header a LEFT JOIN or_details b ON a.trace_no = b.trace_no WHERE a.status = 'Finalized' AND a.doc_date BETWEEN '$dtf' AND '$dt2' AND a.cashtype IN ('3') $searchString) a ORDER BY or_no, so_no;");
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("H6","REFERRING DOCTOR");
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("P6","CHECK SALES");
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("Q6","ADV. PAYMENT");

	for($colheader = 0; $colheader <= 16; $colheader++) { $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($colheader,6)->applyFromArray($headerStyle); }
	...
	while($data = $query->fetch_array(MYSQLI_BOTH)) {
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(7,$row,html_entity_decode($data['physician']));
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(15,$row,$check);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(16,$row,$ap);
		/* NUMBER FORMAT */
		...
		for($contentLoop = 0; $contentLoop <= 16; $contentLoop++) {
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(15,$row,$checkGT);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(16,$row,$adPGT);
	/* NUMBER FORMAT */
