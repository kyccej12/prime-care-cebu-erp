<?php
	...
	list($ydtf) = $con->getArray("SELECT CONCAT(DATE_FORMAT('".$con->formatDate($_GET['dt2'])."','%Y'),'-01-01')");
	$query = $con->dbquery("SELECT DISTINCT a.acct, b.description FROM acctg_gl a LEFT JOIN acctg_accounts b ON a.acct = b.acct_code WHERE b. acct_grp IN ('3','12') AND doc_date BETWEEN '".$con->formatDate($_GET['dtf'])."' AND '".$con->formatDate($_GET['dt2'])."' ORDER BY b.description");

	$row = 8;
	while($data = $query->fetch_array()) {
	
		list($ytd) = $con->getArray("select ifnull(sum(debit-credit),0) from acctg_gl where acct = '$data[acct]' and doc_date between '$ydtf' and '".$con->formatDate($_GET['dt2'])."';");
