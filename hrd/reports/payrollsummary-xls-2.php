<?php
	...
	/* MYSQL QUERY */
		...
		$_ih = $pay->dbquery("SELECT id,dept_name FROM pccpayroll.emp_payslip a INNER JOIN pccpayroll.options_dept b ON a.dept = b.id WHERE period_id = '$cutoff' $fs GROUP BY dept");
	...
	while($data = $_ih->fetch_array(MYSQLI_BOTH)) {
		$a = $pay->dbquery("select * from pccpayroll.emp_payslip where period_id = '$cutoff' and dept = '$data[id]' $fs order by emp_name;");
		...
		while($data2 = $a->fetch_array(MYSQLI_BOTH)) {
			...
			if($cutoff == 23 || $cutoff == 24) {
				list($bonus) = $pay->getArray("select amount from pccpayroll.13th_month where emp_id = '$data2[emp_id]' and file_id = '1';");		
			}
