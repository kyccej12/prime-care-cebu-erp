...
<tbody>';
	...
	while($row = $a->fetch_array(MYSQLI_BOTH)) {
		...
		$html = $html . '<tr>
			...
			<td align="left">' . $row['bday'] . '</td>
			<td align="left">' . $row['dhired'] . '</td>
			<td align="left">' . number_format($row['basic_rate'],2) . '</td>
		...
		$sssGT+=$row['sss_premium']; $sssERGT+=$row['sss_premium_er']; $hdmfGT+=$row['pagibig_premium']; $hdmfERGT+=$row['pagibig_premium_er']; $phGT += $row['philhealth_premium']; $phERGT += $row['philhealth_premium_er']; $wtaxGT += $row['wtax'];
	...
	$html .= '<tr style="background-color: #ededed;">
				...
				<td align="right"><b>' . number_format($wtaxGT,2) . '</b></td>
			</tr>';
$html = $html . '
</tbody>
</table>
