...
<tbody>';
	...
	$_i = $pay->dbquery("SELECT a.emp_id, c.dept_abbrv AS dept, CONCAT(LNAME,', ',FNAME,' ',LEFT(MNAME,1),'.') AS emp_name, DATE_FORMAT(DATE, '%m/%d/%Y %a') AS deyt, DATE, IF(CLOCKOUT!='00:00:00',TIME_FORMAT(CLOCKOUT, '%h:%i'),'') AS out_pm,(REG_OT+SUN_OT) AS tot_ot FROM pccpayroll.emp_dtrfinal a LEFT JOIN pccpayroll.emp_masterfile b ON a.emp_id=b.emp_id LEFT JOIN pccpayroll.options_dept c ON a.dept = c.id WHERE PERIOD_ID = '$_GET[cutoff]' AND OT_APPROVE = 'Y' $f1 $f2 ORDER BY b.lname,a.date ASC;");
