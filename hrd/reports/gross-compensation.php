	...
	<table style="border-collapse: collapse;">	
		...
		<?php
			...
			while($res = $emp_list->fetch_array(MYSQLI_BOTH)) {
				...
				list($usedSIL) = $pay->getArray("SELECT SUM(IF(`length`='WD',1,0.5)) FROM pccpayroll.pay_loa WHERE emp_id = '$res[emp_id]' AND DATE_FORMAT(`date`,'%Y') = '$_GET[year]' AND leave_type IN (1,2);");
				...
				echo '<tr bgcolor='.$pay->initBackground($i).'>
						...
						for($z = 1; $z <= 12; $z++) {
							...
							list($moPay) = $pay->getArray("select sum(gross_pay) from pccpayroll.emp_payslip where period_id in (select period_id from pccpayroll.pay_periods where reportingMonth = '$zmonth' and reportingYear = '$_GET[year]') and emp_id = '$res[emp_id]';");
			...
			echo '<tr bgcolor='.$pay->initBackground($i).'>
						...
						for($z = 1; $z <= 12; $z++) {
							$zmonth = str_pad($z,2,'0',STR_PAD_LEFT);
							list($moPayGT) = $pay->getArray("select sum(gross_pay) from pccpayroll.emp_payslip where period_id in (select period_id from pccpayroll.pay_periods where reportingMonth = '$zmonth' and reportingYear = '$_GET[year]') $f1;");
