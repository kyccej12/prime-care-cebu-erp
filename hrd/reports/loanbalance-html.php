<?php
	...
	/* MYSQL QUERIES SECTION */	
		...
		list($ee_name) = $pay->getArray("select concat('(',emp_id,') ',lname,', ',fname,' ',mname) from pccpayroll.emp_masterfile where emp_id = '$_REQUEST[id_no]';");
		if($_GET['type'] != '') { $searchString .= " and a.loan_type = '$_GET[type]' "; }
		if($_GET['loan_id'] != '') { $searchString .= " and a.record_id = '$_GET[loan_id]' "; }
	/* END OF SQL QUERIES */
...
<body bgcolor="#ffffff" leftmargin="10" bottommargin="50" rightmargin="10" topmargin="10" width="215">	
	...
	<table cellspacing=0 cellpadding=0 border=0 width=100% cellspacing=3 style="font-size: 10px;">	
		...
		<tbody>
			<?php
				//echo "SELECT record_id, LPAD(record_id,6,'0') AS rid, b.loan_type, DATE_FORMAT(date_loan,'%m/%d/%Y') AS loandate, loan_amt, CONCAT(loan_terms,' Mos.') AS terms, remarks FROM pccpayroll.emp_loanmasterfile a LEFT JOIN pccpayroll.option_loantype b ON a.loan_type = b.id WHERE emp_id = '$_REQUEST[id_no]' AND (balance > 0 or balance < 0) AND file_status = 'Active' $f1;";
				$_idetails = $pay->dbquery("SELECT record_id, LPAD(record_id,6,'0') AS rid, b.loan_type, DATE_FORMAT(date_loan,'%m/%d/%Y') AS loandate, loan_amt, CONCAT(loan_terms,' Mos.') AS terms, remarks, a.effective_date FROM pccpayroll.emp_loanmasterfile a LEFT JOIN pccpayroll.option_loantype b ON a.loan_type = b.id WHERE emp_id = '$_REQUEST[id_no]' AND file_status = 'Active' $searchString;");
					while($row = $_idetails->fetch_array(MYSQLI_BOTH)) {
						...
						list($periodStartOfDeduction) = $pay->getArray("SELECT period_id FROM pccpayroll.pay_periods WHERE '$row[effective_date]' >= period_start AND '$row[effective_date]' <= period_end;");

						$_xdetails = $pay->dbquery("SELECT amount AS amount_paid,CONCAT('Payroll: ',DATE_FORMAT(period_start,'%m/%d'),'-',DATE_FORMAT(period_end,'%m/%d'),', ',DATE_FORMAT(period_end,'%Y')) AS reference FROM pccpayroll.emp_deductionmaster a LEFT JOIN pccpayroll.pay_periods b ON a.period_id = b.period_id WHERE ref_id = '$row[record_id]' AND `type` = 'L' AND emp_id = '$_REQUEST[id_no]' and a.period_id >= '$periodStartOfDeduction' order by a.period_id asc;");
