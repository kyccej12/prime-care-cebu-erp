<?php
	...
	$q = $con->dbquery("SELECT emp_id,`name` as emp,bank,annual,amount FROM pccpayroll.thirteenth_month where `year` = '". $_GET['year'] . "' order by `name` asc;");
...
while($res = $q->fetch_array()) {
	...
	list($rate) = $con->getArray("SELECT BASIC_RATE from pccpayroll.emp_masterfile where emp_id = '$res[emp_id]';");
	...
	$html = $html . '<table width="100%">
		...
		<table width="100%" cellspacing=0 cellpadding=0 class=e_info>
			...
			<tr>
				...
				<td width=35% style="padding-left: 5px;">'. number_format($rate,2) . '</td>
				<td width=20%></td>
				<td></td>
				<td width=30% style="padding-left: 5px;"></td>
			</tr>
