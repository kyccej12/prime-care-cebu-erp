<?php
	...
	/* MYSQL QUERIES SECTION */
		...
		$query = $mypayroll->dbquery("SELECT DISTINCT CONCAT('(',a.EMP_ID,') ',lname,', ',fname,' ',mname) AS employee, a.emp_id FROM pccpayroll.emp_dtrfinal a LEFT JOIN pccpayroll.emp_masterfile b ON a.EMP_ID = b.EMP_ID WHERE a.period_id = '$_GET[period]' $f1 ORDER BY b.lname, b.fname;");
		/* END OF SQL QUERIES */
...
while($row = $query->fetch_array()) {
	$a = $mypayroll->dbquery("SELECT DISTINCT emp_id, `date` AS yday, shift, DATE_FORMAT(`date`,'%a %m/%d/%y') AS xday, IF(clockin!='00:00:00',LEFT(clockin,LENGTH(clockin)-3),'') AS in_am,IF(clockout!='00:00:00',LEFT(clockout,LENGTH(clockout)-3),'') AS out_pm, IF(TOT_WORK>0,TOT_WORK,'') AS hrs, IF(TOT_LATE>0,TOT_LATE*60,'') AS late, IF(REG_OT>0,REG_OT,'') AS ot, IF(SUN_OT>0,SUN_OT,'') AS sot, IF(PREM_OT>0,PREM_OT,'') AS pot, (REG_OT+SUN_OT+PREM_OT) AS tot, ot_approve FROM pccpayroll.emp_dtrfinal WHERE emp_id = '$row[emp_id]' AND period_id = '$_GET[period]' ORDER BY `date` ASC;");
	$hGT = 0; $lGT = 0; $otGT = 0;
