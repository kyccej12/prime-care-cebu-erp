...
<tbody>';
	...
	$a = $pay->dbquery("select distinct a.dept, b.dept_name from pccpayroll.emp_payslip a left join pccpayroll.options_dept b on a.dept = b.id where period_id = '$cutoff' $whereString order by b.dept_name;");
	while($drow = $a->fetch_array()) {
		...
		$b = $pay->dbquery("select * from pccpayroll.emp_payslip where period_id = '$cutoff' and dept = '$drow[dept]' and register_exclude != 'Y' order by emp_name;");
		while($row = $b->fetch_array(MYSQLI_BOTH)) {
			...
			$html = $html . '<tr>
				...
				<td align="left">';
					$ltype = $pay->dbquery("SELECT b.loan_type FROM pccpayroll.emp_deductionmaster a LEFT JOIN pccpayroll.option_loantype b ON a.ref_type = b.id WHERE a.emp_id = '$row[emp_id]' AND a.type = 'L' AND period_id = '$cutoff' ORDER BY a.ref_date ASC;");
				...
				$html .= '</td>
				<td align="right">';
					$lamt = $pay->dbquery("SELECT amount FROM pccpayroll.emp_deductionmaster WHERE emp_id = '$row[emp_id]' AND type = 'L' AND period_id = '$cutoff' ORDER BY ref_date ASC;");
				...
				$html .= '</td>
				<td align="left">';
					$otype = $pay->dbquery("SELECT c.deduction_type FROM pccpayroll.emp_deductionmaster a LEFT JOIN pccpayroll.emp_otherdeductions b ON a.ref_id = b.record_id LEFT JOIN pccpayroll.option_deductiontype c ON b.deduction_type = c.id WHERE a.emp_id = '$row[emp_id]' AND a.type = 'O' AND a.period_id = '$cutoff' ORDER BY a.ref_date ASC;");
				...
				$html .= '</td>
				<td align="right">';
					$otamt = $pay->dbquery("SELECT amount FROM pccpayroll.emp_deductionmaster  WHERE emp_id = '$row[emp_id]' AND type = 'O' AND period_id = '$cutoff' ORDER BY ref_date ASC;");
