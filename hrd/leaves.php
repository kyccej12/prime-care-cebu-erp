<?php
	...
	$queryString = "SELECT trans_id AS id, DATE_FORMAT(`date`,'%m/%d/%Y') AS tdate, leave_type AS `type`,`length`, CONCAT(DATE_FORMAT(date_from,'%m/%d/%Y'),' to ',DATE_FORMAT(date_to,'%m/%d/%Y')) AS `range`, a.emp_id, CONCAT(b.lname,', ',fname,' ',LEFT(mname,1),'.') AS `name`, reasons, c.dept_abbrv, address_on_leave FROM pccpayroll.pay_loa a LEFT JOIN pccpayroll.emp_masterfile b ON a.emp_id=b.emp_id LEFT JOIN pccpayroll.options_dept c ON b.dept=c.id WHERE a.file_status = 'Active' $fs order by `date` desc";
				...
				<tr>
					...
					<td align=left>
						<select name="type" id="type" style="width: 80%; font-size: 11px; padding: 5px;">
							<?php 
								$qemp2 = $mydb->dbquery("select * from pccpayroll.options_leavetype;");
