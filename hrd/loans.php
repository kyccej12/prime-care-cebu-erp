			...
			$("#record").dialog({title: "New Record", width: 440, resizable: false, modal: true, buttons: {
				"Save Record": function() {
					...
					if(msg != "") { parent.sendErrorMessage(msg); } else {
						$.post("misc-data.php", { mod: "newLoan", emp_id: $("#emp_id").val(), type: $("#loan_type").val(), loan_date: $("#loan_date").val(), amount: $("#amount").val(), terms: $("#loan_terms").val(), amrtz: $("#semi_amrtz").val(), monthly_amrtz: $("#monthly_amrtz").val(), mplier: $("#mplier").val(), eff: $("#effective_date").val(), dedu_type: $("#dedu_type").val(), active: $("#is_active").val(), ref_no: $("#ref_no").val(), remarks: $("#remarks").val(), sid: Math.random() }, function() {
		...
		<table class="cell-border" id="itemlist" style="font-size:11px;">
			<thead>
				<tr>
					...
					<th width=12%>TERMS (MOS.)</th>
					<th width=10%>AMOUNT</th>
					<th width=10%>AMRTZ'N</th>
					<th width=8%>OFFSET</th>
					<th width=8%>PAID</th>
					<th>BALANCE</th>
					<th></th>
					<th></th>
		...
		<form name="irec" id="irec">
			<table width=100% cellspacing=2 cellpadding=0>
				...
				<tr>
					...
					<td align=left>
						<select class="gridInput" name="loan_type" id="loan_type" style="width: 80%; font-size: 11px; padding: 5px;">
							...
							<?php 
								$l_res = $mydb->dbquery("select id, loan_type from pccpayroll.option_loantype order by loan_type asc;");
				...
				<tr>
					...
					<td align=left>
						<input type="text" name="semi_amrtz" id="semi_amrtz" style="width: 55%;"> * <select class="gridInput" name="mlplier" id="mplier" style="width: 18%; font-size: 11px;"><option value='1'>1</option><option value='2'>2</option><option value='3'>3</option></select>
					</td>
				</tr>
				<tr>
					<td class="bareThin" align=left width=40%>Monthly Amortization:</td>
					<td align=left>
						<input type="text" name="monthly_amrtz" id="monthly_amrtz" style="width: 80%;"> 
					</td>
				</tr>
				<tr>
