<?php
	...
	list($dtf,$dt2,$looper) = $db->getArray("SELECT period_start, period_end, DATEDIFF(period_end,period_start) AS looper FROM pccpayroll.pay_periods WHERE period_id = '$_REQUEST[period]';");
	...
	<tr>
		<td  style="padding:0px;" valign=top>
			<table border="0" cellpadding="0" cellspacing="0" width=100%>
				...
				<?php
					$i = 0;
					$a = $db->dbquery("SELECT a.record_id, a.EMP_ID as eid, CONCAT(b.LNAME,', ',b.FNAME) AS ename, b.SHIFT as shift, `DATE` as xdate, DATE_FORMAT(`DATE`,'%m/%d/%y %a') AS d8, DATE_FORMAT(`DATE`,'%a') as xday, IF(IN_AM!='00:00:00',TIME_FORMAT(IN_AM,'%H:%i'),'') AS `am_in`, IF(OUT_AM!='00:00:00',TIME_FORMAT(OUT_AM,'%H:%i'),'') AS `am_out`, IF(IN_PM!='00:00:00',TIME_FORMAT(IN_PM,'%H:%i'),'') AS `pm_in`, IF(OUT_PM!='00:00:00',TIME_FORMAT(OUT_PM,'%H:%i'),'') AS `pm_out`, IF(TOT_WORK > 0,TOT_WORK,'') AS hrs, IF(TOT_LATE > 0,TOT_LATE,'') AS late, IF(REG_OT > 0,REG_OT,'') AS ot, IF(PREM_OT > 0,PREM_OT,'') AS pot, IF(SUN_OT > 0,SUN_OT,'') AS sot, (reg_ot+sun_ot+prem_ot) AS tot, ot_approve, hd_type FROM pccpayroll.emp_dtrfinal a LEFT JOIN pccpayroll.emp_masterfile b ON a.EMP_ID = b.EMP_ID WHERE b.PAYROLL_TYPE = '$_GET[pay_type]' AND `DATE` BETWEEN '$dtf' AND '$dt2' AND (REG_OT+SUN_OT) >= 0.30 ORDER BY B.LNAME, A.DATE;");
