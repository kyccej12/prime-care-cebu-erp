<?php
	...
	$date_reg = $mydb->formatDate($_POST['date_regularized']); 
	...
	$emp_housing = $mydb->formatDigit($_POST['emp_housing']);
	$emp_hazard = $mydb->formatDigit($_POST['emp_hazard']);
	$emp_comms = $mydb->formatDigit($_POST['emp_comms']);
	$emp_managerial = $mydb->formatDigit($_POST['emp_managerial']);
	...
	if($rid == "") {
		$_xsearch = $mydb->getArray("select count(*) from pccpayroll.emp_masterfile where emp_id = '$emp_idno';");
		if($_xsearch[0] == 0) {
			$_namesearch = $mydb->getArray("select count(*) as found from pccpayroll.emp_masterfile where lname = '$emp_lname' and fname = '$emp_fname' and mname = '$emp_mname';");
			if($_namesearch[0] == 0) { 
			   
			   $query = "INSERT IGNORE INTO pccpayroll.emp_masterfile (EMP_TYPE,EMP_ID,BIO_ID,LNAME,FNAME,MNAME,ADDRESS1,ADDRESS2,GENDER,BIRTHDATE,EMAIL_ADD,TEL_NO,BLOOD_TYPE,
							CIVIL_STATUS,CONTACT_PERSON,CONTACT_NOS,RELIGION,NATIONALITY,DATE_HIRED,DATE_REGULARIZED,DATE_RET,DESG,DEPT,PAYROLL_TYPE,EMPLOYMENT_STATUS,FLEX_TIME,AUTO_NOON,BASIC_RATE,COLA,ALLOWANCE,
							ALLOWANCE_TYPE,NONTAX_ALLOWANCE,TRANSPO_ALLOWANCE,MEAL_ALLOWANCE,HOUSING_ALLOWANCE,MS_ALLOWANCE,HAZARD_ALLOWANCE,COMMS_ALLOWANCE,RETIREMENT_PLAN,COOP_PREMIUM,HDMF_PREMIUM,ACCT_NO,ATM_BANK,W_TAX,VL_CREDIT,SL_CREDIT,TIN_NO,SSS_NO,HDMF_NO,
						...
						VALUES ('$emp_type','$emp_idno','$bio_id','".htmlentities($emp_lname)."','".htmlentities($emp_fname)."','".htmlentities($emp_mname)."','".$mydb->escapeString(htmlentities($emp_add1))."',
							...
							'$nationality','$date_hired','$date_reg','$emp_date_ret','".$mydb->escapeString(htmlentities($emp_desg))."','$emp_dept','$emp_ptype','$emp_stat','$emp_flex','$emp_noon','$emp_rate','$cola','$emp_allw','$emp_allwtype','$emp_allw_ntx',
							'$emp_transpo','$emp_meal','$emp_housing','$emp_managerial','$emp_hazard','$emp_comms','$emp_retirement','$emp_coop','$emp_hdmf_ee','$emp_bank','$emp_atmbank','$emp_wtax','$emp_vl','$emp_sl','$emp_tin','$emp_sss_no','$emp_hdmf_no','$emp_phealth_no',
		...
		if ($old_idno != $emp_idno) { $_xsearch =  $mydb->getArray("select count(*) from pccpayroll.emp_masterfile where emp_id='$emp_idno';"); } else { $_xsearch[0] == 0; }
		if($_xsearch[0] == 0) {
			
			$query = "UPDATE IGNORE pccpayroll.emp_masterfile SET EMP_TYPE = '$emp_type',EMP_ID = '$emp_idno',BIO_ID = '$bio_id',LNAME = '".htmlentities($emp_lname)."',FNAME = '".htmlentities($emp_fname)."',
							...
							CONTACT_NOS = '$contact_nos',RELIGION = '$emp_religion',NATIONALITY = '$nationality',DATE_HIRED = '$date_hired',DATE_REGULARIZED = '$date_reg',DATE_RET = '$emp_date_ret',DESG = '".$mydb->escapeString(htmlentities($emp_desg))."',DEPT = '$emp_dept',
							...
							NONTAX_ALLOWANCE = '$emp_allw_ntx',TRANSPO_ALLOWANCE = '$emp_transpo',MEAL_ALLOWANCE = '$emp_meal',MS_ALLOWANCE = '$emp_managerial',COMMS_ALLOWANCE = '$emp_comms',HAZARD_ALLOWANCE = '$emp_hazard',HOUSING_ALLOWANCE = '$emp_housing',RETIREMENT_PLAN = '$emp_retirement',COOP_PREMIUM = '$emp_coop',HDMF_PREMIUM = '$emp_hdmf_ee',
