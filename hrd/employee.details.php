<?php
	...
	$mydb = new myDB;
	$emp_idno = $_REQUEST['emp_idno'];
	
	if($_REQUEST['isUpload'] == 'Y') {
		...
		$newFileName = $_REQUEST['emp_idno'] . "." . $ext;
		$filePath = $uploadDir . $newFileName;
		
		$check = getimagesize($_FILES["fileToUpload"]["tmp_name"]);
		if($check !== false) {
			$uploadOk = 1;
			$mydb->dbquery("update ignore pccpayroll.emp_masterfile set IMAGE_PATH = '$filePath' where EMP_ID = '$_REQUEST[emp_idno]';");
	...
	if($emp_idno) {
		$myQuery = $mydb->dbquery("select *, date_format(BIRTHDATE,'%m/%d/%Y') as BDAY, date_format(DATE_HIRED,'%m/%d/%Y') as DHIRED, date_format(DATE_REGULARIZED,'%m/%d/%Y') as DREG, if(DATE_RET!='',date_format(DATE_RET,'%m/%d/%Y'),'') as DATE_RETIRED, RELIGION, if((IMAGE_PATH IS NULL OR IMAGE_PATH=''),'photos/noimage.jpg',IMAGE_PATH) as ipath from pccpayroll.emp_masterfile where emp_id='$emp_idno';");
		$_xres = $myQuery->fetch_array();
		$rsNext = $mydb->getArray("select emp_id from pccpayroll.emp_masterfile where record_id > $_xres[record_id] AND FILE_STATUS != 'DELETED' limit 1;");
		$rsPrev = $mydb->getArray("select emp_id from pccpayroll.emp_masterfile where record_id < $_xres[record_id] AND FILE_STATUS != 'DELETED' order by record_id desc limit 1;");
	}
	
	
	if(!$rsNext[0]) {
		$rsNext = $mydb->getArray("select emp_id from pccpayroll.emp_masterfile where FILE_STATUS != 'DELETED' order by record_id asc limit 1;");
	}
	if(!$rsPrev[0]) {
		$rsPrev = $mydb->getArray("select emp_id from pccpayroll.emp_masterfile where FILE_STATUS != 'DELETED' order by record_id desc limit 1;");
	}
?>
<html>
<head>
	...
	<script language="javascript" src="../js/hrd-new.js?sessid=<?php echo uniqid(); ?>"></script>
	...
	<script language="javascript">
		...
		$(document).ready(function($){
			$("#date_hired").datepicker({ changeMonth: true, changeYear: true }); 
			$("#emp_bday").datepicker({ changeMonth: true, changeYear: true }); 
			$("#date_ret").datepicker({ changeMonth: true, changeYear: true });
			<?php if($_xres['FILE_STATUS'] == 'DELETED'){ echo "$(\"#empdetails :input\").prop('disabled',true);"; } ?>
		...
		function uploadImage() {
			$("#uploadImageDiv").dialog({
				title: "Update Employee ID Picture",
				width: 400,
				resizable: false,
				modal: true,
				buttons: {
					"Upload ID Now": function() {
						if($("#fileToUpload").val() == '') {
							parent.sendErrorMessage("Nothing to Upload. Makde sure you have selected a product image to upload..");
						} else {
							document.uploadImageFrm.submit();
						}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" >
 <form name="empdetails" id="empdetails">
				...
				<tr>
					<td align=left>
						<?php
							...
							if($_xres['record_id'] > 0 && $_xres['FILE_STATUS'] == 'ACTIVE') {
								$topButtons .= '<a href="#" class="topClickers" onclick="javacript: movePrev()"><img src="../images/icons/previous.png" width=12 height=12 align=absmiddle />&nbsp;Previous Record</a>&nbsp;';
							}
						
							if($_xres['FILE_STATUS'] == 'ACTIVE' || $_xres['FILE_STATUS'] == '') {
								$topButtons .=  '<a href="#" class="topClickers" onclick="javascript: save_record()"><img src="../images/icons/floppy.png" width=12 height=12 align=absmiddle />&nbsp;Save Record</a>&nbsp;';
							} else {
								$topButtons .=  '<a href="#" class="topClickers" onclick="javascript: restore_record(\''.$_xres['record_id'].'\',\''.$_xres['EMP_ID'].'\');"><img src="../images/icons/refresh.png" width=12 height=12 align=absmiddle />&nbsp;Restore Record</a>&nbsp;';
							}
							
							if($_xres['record_id'] > 0 && $_xres['FILE_STATUS'] == 'ACTIVE') {
								$topButtons .= '<a href="#" class="topClickers" onclick="javascript: delete_record('. $_xres['record_id'] . ');"><img src="../images/icons/bin.png" width=12 height=12 align=absmiddle />&nbsp;Archive Record</a>&nbsp;';
								$topButtons .= '<a href="#" class="topClickers" onclick="javascript: moveNext()"><img src="../images/icons/forward.png" width=12 height=12 align=absmiddle />&nbsp;Next Record</a>';
							}
			...
			<table width=99% align=center cellpadding=0 cellspacing=0 style="border: thin solid #ccc;">
				<tr bgcolor="#f5f5f5">
						...
						<table border="0" cellpadding="0" cellspacing="0" width=100%>
							...
							<tr>
								...
								<td class=bare width=30%>
									<select class="gridInput" name="emp_type" id="emp_type" style="width:75%">
										<option value=''>- Select -</option>
										<?php
											$etypeQuery = $mydb->dbquery("select eTypeID,eType from pccpayroll.option_emptype;");
						...
						<table border="0" cellpadding="0" cellspacing="0" width=100%>
							...
							<tr>
								<td class=bareThin align="left" width=30%>Gender :&nbsp;&nbsp</td>
									...
									<select class="gridInput" id="emp_sex" name="emp_sex" style="width : 140px">
										<option value=""> - Select -</option>
							...
							<tr>
								...
								<td class=bare align="left">
									<select class="gridInput" id="emp_bloodtype" name="emp_bloodtype" style="width : 140px">
									<option value=''>- Not Specified -</option>
									<?php
										$btQuery = $mydb->dbquery("select bloodType from pccpayroll.option_bloodtypes;");
							...
							<tr>
								...
								<td class=bare align="left">
									<select class="gridInput" id="emp_cstat" name="emp_cstat" style="width : 140px">
										<?php
											$csQuery = $mydb->dbquery("select csid,civil_status from pccpayroll.options_civilstatus");
							...
							<tr>
								...
								<td class=bare>
									<select class="gridInput" id="emp_nationality" name="emp_nationality" style="width : 140px">
										<?php
											$natQuery = $mydb->dbquery("select line_id, nation_desc from pccpayroll.nationality order by nation_desc;");
					...
					<td valign=top style="padding-top: 10px;">
						<?php if($_REQUEST['emp_idno']!='') { ?>
							<image src="<?php echo $_xres['ipath']; ?>" style = "border: 1px solid black;" width = 180 height = 180 /><br/>
							<button type="button" onclick="javascript: uploadImage();" style="margin-top: 5px; width: 180px; height: 30px;"><img src="../images/icons/camera.png" width=16 height=16 align=absmiddle>&nbsp;Upload Employee ID</button>
						<?php } ?>
					</td>
				</tr>	
			</table>
				...
				<tr bgcolor="#f5f5f5">
					<td width=50% valign=top style="padding: 10px">
						<table border="0" cellpadding="0" cellspacing="0" width=100%>
							...
							<tr><td height=2></td></tr>
							<tr>
								<td class="bareThin" width="25%">Date Regularized :&nbsp;&nbsp;</td>
								<td class="bare" align="left">
									<input type="text" id="date_regularized" name="date_regularized" style="width : 140px" value="<?php echo $_xres['DREG'] ?>">
								</td>
							</tr>
							<tr><td height=2></td></tr>
							<tr>
							...
							<tr>
								...
								<td class="bare" align="left">
									<select class="gridInput" id="emp_dept" name="emp_dept" style="width : 300px">
										<?php
											$dept = $mydb->dbquery("select id,dept_name from pccpayroll.options_dept order by id;");
							...
							<tr>
								...
								<td class="bare" align="left">
									<select class="gridInput" id="emp_ptype" name="emp_ptype" style="width : 160px">
										<?php
											$ptype = $mydb->dbquery("select id,payroll_type from pccpayroll.payroll_type order by payroll_type;");
							...
							<tr>
								...
								<td class="bare" align="left">
									<select class="gridInput" id="emp_stat" name="emp_stat" style="width : 160px">
										<?php
											$estatus = $mydb->dbquery("select id,emp_status from pccpayroll.emp_status order by id;");
							...
							<tr>
								...
								<td class="bare" align="left">
									<select class="gridInput" id="emp_flex" name="emp_flex" style="width : 100px">
							...
							<tr>
								...
								<td class="bare" align="left">
									<select class="gridInput" id="emp_batch" name="emp_batch" style="width : 100px">
										<?php
											$_sq = $mydb->dbquery("select batch_id,batch_details from pccpayroll.pay_batch order by batch_id");
							...
							<tr>
								...
								<td class="bare" align="left">
									<select class="gridInput" id="emp_shift" name="emp_shift" style="width : 100px">
										<?php
											$_sq = $mydb->dbquery("select distinct shift_id, remarks from pccpayroll.emp_shifts order by shift_id");
											while($sqrow = $_sq->fetch_array(MYSQLI_BOTH)) {
												echo "<option value='$sqrow[0]' title='$sqrow[1]' ";
							...
							<tr>
								...
								<td class="bare" align="left">
									<select class="gridInput" class="gridInput" id="emp_area" name="emp_area" style="width : 120px">
										<?php
											$_sa1 = $mydb->dbquery("select `area`, `region` from pccpayroll.emp_areas");
							...
							<tr>
								...
								<td class="bare" align="left">
									...
									<select class="gridInput" name="emp_sss" id="emp_sss" style="width: 40px;">
							...
							<tr>
								...
								<td class="bare" align="left">
									...
									<select class="gridInput" name="emp_ph" id="emp_ph" style="width: 40px;">
							...
							<tr>
								...
								<td class="bare" align="left">
									...
									<select class="gridInput" name="emp_hdmf" id="emp_hdmf" style="width: 40px;">
					...
					<td  style="padding: 10px;" valign="top">	
						<table border="0" cellpadding="0" cellspacing="0" width="100%">
							...
							<tr>
								...
								<td class="bare" align="left">
									<select class="gridInput" name="emp_allwtype" id="emp_allwtype" style="width: 100px;">
							...
							<tr><td height=2></td></tr>
							<tr>
								<td class="bareThin" width="35%" align="left">Managerial/Supervisory Allowance :&nbsp;&nbsp;</td>
								<td class="bare" align="left">
									<input type="text" id="emp_managerial" name="emp_managerial" style="width : 100px" value="<?php echo number_format($_xres['MS_ALLOWANCE'],2) ?>">
								</td>
							</tr>
							<tr><td height=2></td></tr>
							<tr>
								<td class="bareThin" width="35%" align="left">Housing Allowance :&nbsp;&nbsp;</td>
								<td class="bare" align="left">
									<input type="text" id="emp_housing" name="emp_housing" style="width : 100px" value="<?php echo number_format($_xres['HOUSING_ALLOWANCE'],2) ?>">
								</td>
							</tr>
							<tr><td height=2></td></tr>
							<tr>
								<td class="bareThin" width="35%" align="left">Communication Allowance :&nbsp;&nbsp;</td>
								<td class="bare" align="left">
									<input type="text" id="emp_comms" name="emp_comms" style="width : 100px" value="<?php echo number_format($_xres['COMMS_ALLOWANCE'],2) ?>">
								</td>
							</tr>
							<tr><td height=2></td></tr>
							<tr>
								<td class="bareThin" width="35%" align="left">Transpo Allowance :&nbsp;&nbsp;</td>
								<td class="bare" align="left">
									<input type="text" id="emp_transpo" name="emp_transpo" style="width : 100px" value="<?php echo number_format($_xres['TRANSPO_ALLOWANCE'],2) ?>">
								</td>
							</tr>
							<tr><td height=2></td></tr>
							<tr>
								<td class="bareThin" width="35%" align="left">Hazard Pay :&nbsp;&nbsp;</td>
								<td class="bare" align="left">
									<input type="text" id="emp_hazard" name="emp_hazard" style="width : 100px" value="<?php echo number_format($_xres['HAZARD_ALLOWANCE'],2) ?>">
								</td>
							</tr>
							<tr><td height=2></td></tr>
							<tr>
							...
							<tr>
								...
								<td class="bare" align="left">
									<select class="gridInput" name="emp_ewt" id="emp_ewt" style="width: 100px;">
							...
							<tr>
								...
								<td class="bare" align="left">
									<select class="gridInput" name="emp_atmbank" id="emp_atmbank" style="width: 160px;">
										<option value='0'>- Cash -</option>
										<?php
											$atm = $mydb->dbquery("select bank_id, bank_name from pccpayroll.options_paybank order by bank_name");
							...
							<tr>
								...
								<td class="bare" align="left">
									<input type="text" id="emp_bank" name="emp_bank" style="width : 160px" value="<?php echo $_xres['ACCT_NO'] ?>">
								</td>
							</tr>
							<tr><td height=2></td></tr>
							<tr>
								...
								<td class="bare" align="left">
									<select class="gridInput" name="emp_expayreg" id="emp_expayreg" style="width: 100px;">
	...
	<div id="uploadImageDiv" style="display: none;">
		<form name="uploadImageFrm" id="uploadImageFrm" enctype="multipart/form-data" action="employee.details.php" method="POST">
			<input type="hidden" name="emp_idno" value="<?php echo $_REQUEST['emp_idno']; ?>">
			<input type="hidden" name="isUpload" value="Y">
			<span class="spandix-l">Select image to upload:</span></br>
