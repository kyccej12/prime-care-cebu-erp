		...
		foreach($lines as $key => $value){
			...
			if($data[0]  != '') {
				if($data[1] != '') {
					...
					list($area) = $con->getArray("SELECT `AREA` FROM pccpayroll.emp_masterfile WHERE trim(LEADING '0' from ACCT_NO) = '$bdoAcct';");
				} else {
					$einfo = explode(',',$data[0]);
					list($area) = $con->getArray("select `AREA` from pccpayroll.emp_masterfile where lname like '%$einfo[0]%' and fname like '%".trim($einfo[1])."%';");
				}	
				$txtstring = "INSERT IGNORE INTO pccpayroll.thirteenth_month (`name`,bank,amount,`year`,`area`) VALUES ('$data[0]','".str_pad($data[1],12,'0',STR_PAD_LEFT)."','".$con->formatDigit($data[2])."','$_POST[fiscal_year]','$area');";
