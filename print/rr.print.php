...
/* MYSQL QUERIES SECTION */
	...
	$_ihead = $con->getArray("select trace_no, lpad(rr_no,6,0) as rr, date_format(rr_date,'%m/%d/%Y') as d8, supplier, supplier_name, supplier_addr, invoice_no, if(invoice_date!='',if(invoice_date='0000-00-00','',date_format(invoice_date,'%m/%d/%Y')),date_format(invoice_date,'%m/%d/%Y')) as id8, received_by, amount, remarks from rr_header where rr_no='$_REQUEST[rr_no]' and branch = '$_SESSION[branchid]';");
	$_idetails = $con->dbquery("select costcenter, if(po_no='','--',lpad(po_no,6,0)) as po_no, if(po_date='0000-00-00','--',date_format(po_date,'%m/%d/%y')) as po_date, item_code, description, qty, unit, lot_no, if(expiry!='0000-00-00',date_format(expiry,'%m/%d/%Y'),'') as expiry, cost, amount from rr_details where rr_no = '$_REQUEST[rr_no]' and branch = '$_SESSION[branchid]';");
	$bcode = $_ihead['trace_no'];
	list($nos, $stin) = $con->getArray("select tel_no, tin_no from contact_info where file_id = '$_ihead[supplier]';");

	list($dCount) = $con->getArray("select count(*) from rr_details where rr_no = '$_REQUEST[rr_no]' and branch = '$_SESSION[branchid]';");
...
<tr>
	<td width=25% align=center><b>PREPARED BY:</b><br><br>'.$con->getUname($_SESSION['userid']).'<br></td>
	<td width=25% align=center><b>RECEIVED & CHECKED BY:</b><br><br>'.$_ihead['received_by'].'<br>________________________________<br/><font size=3>Signature over Printed Name</font></td>
	<td width=25%  align=center><b>APPROVED BY:</b><br><br><br>________________________________<br><font size=3>Signature over Printed Name</font></td>
</tr>
</table>
<table width=100%>
...
<thead>
<tr>
<td width="10%" align=left><b>UNIT CODE</b></td>
<td width="7%" align=center><b>PO #</b></td>
<td width="8%" align=center><b>PO DATE</b></td>
<td align=left><b>PARTICULARS</b></td>
<td width="10%" align=right><b>QTY</b></td>
<td width="6%"><b>UoM</b></td>
<td width="8%"><b>LOT #</b></td>
<td width="10%"><b>EXPIRY</b></td>
...
<tbody>';
	...
	while($row = $_idetails->fetch_array(MYSQLI_ASSOC)) {
		...
		$html = $html . '<tr>
		<td align="left">' . $con->identCostCenter($row['costcenter']) . '</td>
