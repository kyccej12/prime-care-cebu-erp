...
/* MYSQL QUERIES SECTION */
	...
	$_idetails = $mydb->dbquery("select item_code, description, qty, unit, cost, amount, lotno, date_format(expiry_date,'%m/%d/%Y') as expiry_date from pharma_sw_details where sw_no = '$_REQUEST[sw_no]' and branch = '$_SESSION[branchid]';");
	$bcode = $_ihead['trace_no'];
	
/* END OF SQL QUERIES */

$mpdf=new mPDF('win-1252','FOLIO-H','','',10,10,52,30,10,10);
...
<table width="100%" cellpadding=0 cellspacing=0>
	<tr>
		...
		<td style="color:#000000; padding-top: 5px;">
...
<thead>
	<tr>
		...
		<td width="10%" align=right><b>LOT NO.</b></td>
		<td width="10%" align=right><b>EXPIRY</b></td>
	</tr>
</thead>
<tbody>';
	$i = 0;
	while($row =$_idetails->fetch_array()) {
		...
		<td align="right">' . $row['lotno'] . '</td>
		<td align="right">' . $row['expiry_date'] . '</td>
		</tr>'; $i++; 
		//$amtGT+=$row['amount'];
	}
