...
/* MYSQL QUERIES SECTION */
	...
	$_ihead = $con->getArray("select lpad(j_no,6,0) as jno, date_format(j_date,'%m/%d/%Y') as jd8, explanation from journal_header where j_no = '$_REQUEST[j_no]' and company = '$_SESSION[company]' and branch = '$_SESSION[branchid]';");
	$_idetails = $con->dbquery("SELECT ref_no, date_format(ref_date,'%m/%d/%y') as ref_date, ref_type, CONCAT('(',`client`,') ',b.tradename) AS cust, acct, acct_desc, cost_center,debit,credit FROM journal_details a LEFT JOIN contact_info b ON a.client=b.file_id WHERE j_no = '$_REQUEST[j_no]' and a.company='$_SESSION[company]' and a.branch='$_SESSION[branchid]' order by debit, acct_desc;");
...
<td class="td-r-top"><b>' . $bit['branch_name'] . '</b></td>
</tr>
<tr>
