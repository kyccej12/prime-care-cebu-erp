<?php
	...
	session_start();
	//list($_GET[eid]) = getArray("select EMP_ID from pccpayroll.emp_masterfile where EMP_ID = '$_GET[eid]';");
	$_sres = getArray("select *, date_format(bday, '%m/%d/%Y') as sbday from pccpayroll.emp_srecord where emp_id = '$_GET[eid]';");
	$_pres = getArray("select *, date_format(mom_bday,'%m/%d/%Y') as mbday, date_format(dad_bday,'%m/%d/%Y') as dbday from pccpayroll.emp_precord where emp_id = '$_GET[eid]';");
?>
<html>
<head>
...
<body bgcolor="#ffffff" leftmargin="0" bottommargin="0" rightmargin="0" topmargin="0">
	...
	<div id="crecords">
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
			<?php
				$check_crecord = mysql_fetch_array(mysql_query("select count(*) from pccpayroll.emp_crecord where emp_id = '$_GET[eid]';"));
				if($check_crecord[0] > 0) {
					...
					$crecords = mysql_query("select record_id, fname, mname, lname, date_format(bday,'%m/%d/%Y') as bday, bday as bd8, gender, status, occupation from pccpayroll.emp_crecord where emp_id = '$_GET[eid]' order by bd8 asc;");
					$x = 1;
	...
	<div id="brecords">
		<table border="0" cellpadding="0" cellspacing="1" width="100%">
		<?php
			$check_brecord = mysql_fetch_array(mysql_query("select count(*) from pccpayroll.emp_brecord where emp_id='$_GET[eid]';"));
			if($check_brecord[0] > 0) {
				...
				$brecords = mysql_query("select record_id, fname, mname, lname, date_format(bday,'%m/%d/%Y') as bday, bday as bd8, gender, status, occupation from pccpayroll.emp_brecord where emp_id = '$_GET[eid]' order by bd8 asc;");
				$i = 1;
