<?php
	...
	switch($_POST['mod']){
		case "saveHeader":
			...
			if($_POST['j_no'] !='') {
				$p->dbquery("update IGNORE journal_header set cy = '".$p->formatCY($_POST['j_date'])."', j_date = '".$p->formatDate($_POST['j_date'])."', explanation = '".$p->escapeString($_POST['remarks'])."', updated_by='$_SESSION[userid]', updated_on = now() where j_no = '$j_no' and branch = '1';");
			} else {
				list($j_no) = $p->getArray("select ifnull(max(j_no),0)+1 from journal_header where branch = '1';"); 
				$p->dbquery("insert ignore into journal_header (branch,cy,j_no, j_date, explanation, created_by,created_on) values ('1','".$p->formatCY($_POST['j_date'])."','$j_no','".$p->formatDate($_POST['j_date'])."','".$p->escapeString($_POST['remarks'])."','$_SESSION[userid]',now());");
			}
		...
		case "addInvoice":
			...
			if($ewt > 0) { $p->dbquery("insert ignore into journal_details (branch,cy,j_no,ref_no,ref_date,ref_type,acct,acct_desc,credit,cost_center) values ('1','".$p->formatCY($_POST['j_date'])."','$j_no','$_POST[ref_no]','".$p->formatDate($_POST['ref_date'])."','SI','30207','".$p->getAcctDesc("30207",1)."','$ewt','$_POST[costcenter]');"); }
			if($vat > 0) { $p->dbquery("insert ignore into journal_details (branch,cy,j_no,ref_no,ref_date,ref_type,acct,acct_desc,debit,cost_center) values ('1','".$p->formatCY($_POST['j_date'])."','$j_no','$_POST[ref_no]','".$p->formatDate($_POST['ref_date'])."','SI','30208','".$p->getAcctDesc("30208",1)."','$vat','$_POST[costcenter]');"); }
			if($_POST['dbAcct'] != '') {
				$p->dbquery("insert ignore into journal_details (branch,cy,j_no,ref_no,ref_date,ref_type,acct,acct_desc,debit,cost_center) values ('1','".$p->formatCY($_POST['j_date'])."','$j_no','$_POST[ref_no]','".$p->formatDate($_POST['ref_date'])."','SI','".$_POST['dbAcct']."','".$p->getAcctDesc($_POST['dbAcct'],1)."','$net','$_POST[costcenter]');");
			}
