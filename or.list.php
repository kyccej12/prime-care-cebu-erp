	...
	<script>
		...
		$(document).ready(function() {
			...
			var myTable = $('#itemlist').DataTable({
				...
				"bSort": false,
				"aoColumnDefs": [
					{ className: "dt-body-center", "targets": [0,1,2,3,5,9] },
		...
		<tbody>
			<?php
				...
				if($_REQUEST['searchtext'] && $_REQUEST['searchtext'] != '') {
					...
					$searchString .= " and ($inSO || or_no like '%$_REQUEST[searchtext]%' || scpwd_id = '$_REQUEST[searchtext]' || customer_name like '%$term%' || remarks like '%$term%' || checkno like '%$_REQUEST[searchtext]%') ";
				...
				while($row = $_i->fetch_array()) {
					...
					$so = '';
					$dQuery = $con->dbquery("SELECT DISTINCT so_no FROM or_details WHERE doc_no = '$row[doc_no]';");	
