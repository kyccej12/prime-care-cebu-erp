<?php
	...
	if(isset($_POST['upload'])) {
		...
		$query = "insert ignore into pccpayroll.emp_certificates (emp_id, doc_title, doc_date, doc_description, filename, filetype, filesize, filepath, date_uploaded) values ('$emp_idno','$_POST[doc_title]','".formatDate($_POST['doc_date'])."','".mysql_real_escape_string($_POST['doc_description'])."','$newFileName','$fileType','$fileSize','$filePath',now());";
		mysql_query($query);
		...
		<td  style="padding:0px;" valign=top>
			...
			<div id="details" style="height:390px; overflow: auto;">
				...
				<?php
					...
					$getRec = dbquery("select record_id, doc_title, date_format(doc_date,'%m/%d/%Y') as doc_date, doc_description, date_format(date_uploaded,'%m/%d/%Y %r') as uploaded from pccpayroll.emp_certificates where emp_id='$emp_idno';");
