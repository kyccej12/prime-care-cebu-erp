<?php
	...
	switch($_REQUEST['mod']) {
		...
		case "addItem":
			...
			if($isE > 0) {
				$s = "update ignore pharma_sw_details set qty = qty + ".$con->formatDigit($_POST['qty']).", lotno = '$_POST[lotno]', expiry_date = '".$con->formatDate($_POST['expiry'])."', amount = amount + ".$con->formatDigit($_POST['amount'])." where trace_no = '$_POST[trace_no]' and item_code = '$_POST[item]' and branch = '$_SESSION[branchid]';";
			} else {
				$s = "insert ignore into pharma_sw_details (branch,sw_no,item_code,description,qty,unit,lotno,expiry_date,cost,amount,trace_no) values ('$_SESSION[branchid]','$_POST[sw_no]','$_POST[item]','".$con->escapeString(htmlentities($_POST['description']))."','".$con->formatDigit($_POST['qty'])."','$_POST[unit]','$_POST[lotno]','".$con->formatDate($_POST['expiry'])."','".$con->formatDigit($_POST['cost'])."','".$con->formatDigit($_POST['amount'])."','$_POST[trace_no]');";
			}
		...
		case "updateItem":
			$con->dbquery("update pharma_sw_details set item_code = '$_POST[item]', description = '".$con->escapeString(htmlentities($_POST['description']))."', qty = '".$con->formatDigit($_POST['qty'])."', unit = '$_POST[unit]',cost = '".$con->formatDigit($_POST['cost'])."', amount = '".$con->formatDigit($_POST['amount'])."', lotno = '$_POST[lotno]', expiry_date = '".$con->formatDate($_POST['expiry'])."'  where line_id = '$_POST[lid]';");
			echo "update pharma_sw_details set item_code = '$_POST[item]', description = '".$con->escapeString(htmlentities($_POST['description']))."', qty = '".$con->formatDigit($_POST['qty'])."', unit = '$_POST[unit]',cost = '".$con->formatDigit($_POST['cost'])."', amount = '".$con->formatDigit($_POST['amount'])."', lotno = '$_POST[lotno]', expiry_date = '".$con->formatDate($_POST['expiry'])."'  where line_id = '$_POST[lid]';";
			updateAmount($_POST['sw_no']);
		break;
		...
		case "retrieve":
			...
			$query = $con->dbquery("SELECT line_id as id, description, item_code, unit, qty, cost, amount, lotno, `expiry_date` FROM pharma_sw_details where trace_no = '$_REQUEST[trace_no]';");
