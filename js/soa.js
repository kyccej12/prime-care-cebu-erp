...
function addItem() {
				...
				click: function() { 
					...
					if(msg != '') {
						...
						$.post("soa.datacontrol.php", { 
                            ...
                            refno: $("#itemReference").val(),
                            date: $("#itemAvailed").val(), 
							item: $("#itemCode").val(), 
	...
	$('#itemDescription').autocomplete({
		...
		select: function(event,ui) {
			...
			$("#itemCost").val(ui.item.specialprice);
...
function cancelSOA() {
    if(confirm("Are you sure you want to cancel this document?") == true) {
        $.post("soa.datacontrol.php", { mod: "cancel", soa_no: $("#soa_no").val(), sid: Math.random() }, function() {
            parent.showSOA($("#soa_no").val());
        });
    }
}

function print() {
    //if($("#revenue_center").val() == '') { var tag = 1; } else { var tag = 2; }
    
    var tag = $("#revenue_center").val();
    var source = $("#source_type").val();
    parent.printSOA($("#soa_no").val(),tag,source);
}