...
function updateItem(){
	...
	if(!id) {
		...
		$("#itemSO").val(sono);
		$("#itemSOA").val(soa_no);
		$("#itemPatient").val(pname);
		$("#itemProc").val(procedure);
		$("#itemPrice").val(unit_price);
		$("#itemDiscount").val(disc);
		$("#itemAmountDue").val(amount);

		var dis = $("#updateEntry").dialog({title: "Update Item", width: 480, resizable: false, modal: true, buttons: [
			{ 
						...
						if(msg != '') {
							...
							if(confirm("Are you sure you want to save changes made to this entry?") == true) {
								$.post("or.datacontrol.php", { 
									...
									procedure: $("#itemProc").val(),
									unit_price: $("#itemPrice").val(),
									disc: $("#itemDiscount").val(),
									amount: $("#itemAmountDue").val(),
									sid: Math.random() },
								function() {
									redrawDataTable();
									dis.dialog("close");
			...
			},
			{
				...
				click: function() { $(this).dialog("close"); $("#frmUpdateEntry").trigger("reset"); }
			}]
		});
	}
}
...
function getTotals() {
	$.post("or.datacontrol.php", { mod: "getTotals", doc_no: $("#doc_no").val(), sid: Math.random() }, function(data) {
		...
		$("#netofvat").val(data['nvat']);
		$("#nonvat").val(data['nonvat']);
		$("#vat").val(data['vat']);
		$("#ewt").val(data['ewt']);
...
function computeEWT(code) {
	$.post("or.datacontrol.php", { mod: "updateEWT", doc_no: $("#doc_no").val(), cid: $("#customer_code").val(), ecode: code, sid: Math.random() }, function() {
		getTotals();
	});
}
...
function deleteItem(){
	...
	if(!lineid) {
		...
		if(confirm("Are you sure you want to remove this line entry?") == true) {
			$.post("or.datacontrol.php", { mod: "deleteLine", lid: lineid, doc_no: $("#doc_no").val(), cid: $("#customer_code").val(), scid: $("#scpwd_id").val(), sid: Math.random() }, function(gt) { redrawDataTable(); });
		}
	}
}
		...
		buttons:[ 
			{
				...
				click: function() {
					...
					var cc = parseFloat(parent.stripComma($("#ccTendered").val()));
					var cash = parseFloat(parent.stripComma($("#cashTendered").val()));
					var check =  parseFloat(parent.stripComma($("#checkTendered").val()));
					
					var tendered = cc + cash + check;
						tendered = tendered.toFixed(2);

					var msg = "";

					if(tendered < due) {
						msg = msg + "Total payments entered is insufficient! Make sure to correctly breakdown payments received before finalizing this document.<br>";
					}

					if(check > 0) {
						if(cash_type == 3) {
							if($("#ck_bank").val() == '') { msg = msg + "- Please specify the check's issuing bank."; }
					...
					if(cc > 0) {
						...
						if($("#cc_approvalno").val() == '') { msg = msg + "- Please specify the transaction's <b>Approval No.</b><br/>"; }
					}
					...
					if(msg!="") {
						...
						if(confirm("Are you sure you want to proceed?") == true) {
							$.post("or.datacontrol.php", {
								...
								doc_no: $("#doc_no").val(),
								paid: $("#balanceDue").val(),
								cash: $("#cashTendered").val(),
								cc: $("#ccTendered").val(),
								check: $("#checkTendered").val(),
...
function computeChange(myId) {
	
	var curInput = document.getElementById(myId).value;


	if(isNaN(curInput) == true) {
		parent.sendErrorMessage("Invalid Amount Entered!");
		$("#changeDue").val('');
		document.getElementById(myId).value = '0.00';
	} else {
		var due = parseFloat(parent.stripComma($("#balanceDue").val()));
		var cc = parseFloat(parent.stripComma($("#ccTendered").val()));
		var cash = parseFloat(parent.stripComma($("#cashTendered").val()));
		...
		if(tendered >= due) {
			...
			parent.sendErrorMessage("Total payments entered is insufficient! Make sure to correctly breakdown payments received before finalizing this document.");
		}
...
function reopen() {
	if(confirm("Are you sure you want to set this document to \"Active\" status?") == true) {
		$.post("or.datacontrol.php",{ mod: "reopen", doc_no: $("#doc_no").val(), cid: $("#customer_code").val(), sid: Math.random() }, function() { location.reload(); });
	}
}

function cancel() {
	if(confirm("Are you sure you want to cancel this document?") == true) {
		$.post("or.datacontrol.php",{ mod: "cancel", doc_no: $("#doc_no").val(), sid: Math.random() }, function() { location.reload(); });
	}
}

function reuse() {
	if(confirm("Are you sure you want to recycle this document?") == true) {
		$.post("or.datacontrol.php",{ mod: "reuse", doc_no: $("#doc_no").val(), sid: Math.random() }, function() { location.reload(); });
	}
}