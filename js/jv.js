...
function applyInvoice() {
	...
	if($("#app_invoice_docno").val() == "") { msg = msg + "- You must specify Invoice or Reference # before applying this document to CV...<br/>"; }
		...
		$.post("jv.datacontrol.php", { mod: "addInvoice", j_no: $("#j_no").val(), j_date: $("#j_date").val(), sname: $("#app_invoice_name").val(), saddr: $("#app_invoice_address").val(), stin: $("#app_invoice_tin").val(), ref_no: $("#app_invoice_docno").val(), ref_date: $("#app_invoice_docdate").val(), amount: amt, dbAcct: $("#app_debit_acct").val(), costcenter: $("#app_costcenter").val(), isVat: $("#app_vatable").val(), atc: $("#app_atc").val(), sid: Math.random() }, function(data) {
			$("#jdetails").html(data);
