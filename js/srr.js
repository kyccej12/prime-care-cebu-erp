		...
		var dis = $("#discounter").dialog({
			...
			buttons: [
				{ 
					text: "Apply Discount",
					click: function() { 
						$.post("srr.datacontrol.php", { mod: "applyLineDiscount", lid: arr[0], srr_no: $("#srr_no").val(), discPercent: $("#discountPercent").val(), sid: Math.random() }, function(gt) {
							redrawDataTable(); 
							$("#grandTotal").val(gt);
							dis.dialog("close");
						});
