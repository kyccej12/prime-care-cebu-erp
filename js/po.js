...
function getTotals() {
	$.post("po.datacontrol.php", { mod: "getTotals", po_no: $("#po_no").val(), sid: Math.random() }, function(data) {
		$("#total_due").val(data['gross']);
		$("#total_discount").val(data['discount']);
		$("#net_due").val(data['net']);
	},"json");
}
...
function savePOHeader() {
	...
	if(msg != "") {
		...
		$.post("po.datacontrol.php", { mod: "saveHeader", trace_no: $("#trace_no").val(), po_no: $("#po_no").val(), po_date: $("#po_date").val(), terms: $("#terms").val(), cid: $("#customer_id").val(), cname: $("#customer_name").val(), addr: $("#cust_address").val(), requested_by: $("#requested_by").val(), costcenter: $("#costcenter").val(), mrs: $("#mrs_no").val(), del_addr: $("#delivery_address").val(), date_needed: $("#date_needed").val(), remarks: $("#remarks").val(), sid: Math.random() },function(pono){
			$("#po_no").val(pono);
			parent.popSaver();
		},"html");
	}
...
function computeItemAmount() {
	...
	var discount = parseFloat(parent.stripComma($("#itemDiscount").val()));


	if(isNaN(qty) == true) { msg = msg + "- Invalid Quantity!"; }
	if(isNaN(cost) == true) { msg = msg + "- Invalid Unit Cost!"; }
	if(isNaN(discount) == true) {  msg = msg + "- Invalid Discount!"; }
...
function updateItem(){
	...
	if(!id) {
		...
		$("#itemUnit").val(unit);
		$("#itemQty").val(qty);
		$("#itemCost").val(parent.kSeparator(cost));
