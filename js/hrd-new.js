...
function restore_record(rid,empid) {
	if(confirm("Are you sure you want to restore this record to Active Status?") == true) {
		$.post("misc-data.php", { mod: "restoreEmployee", rid: rid, sid: Math.random() }, function() {
			alert("Record successfully restored!");
			parent.showEmpProfile(empid);
		});
	}
}