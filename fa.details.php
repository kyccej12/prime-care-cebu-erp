	...
	<script>
		function fa_save() {
			...
			if(msg!="") {
				...
				$.post("src/sjerp.php", { mod: "checkDupAssetNo", fid: $("#fid").val(), asset_no: $("#asset_no").val(), sid: Math.random() }, function(data) {
					if(data == "NODUPLICATE") {
		...
		$(function() { $("#date_assigned").datepicker(); $("#cv_date").datepicker(); $("#acquired_date").datepicker(); $("#check_date").datepicker(); $("#warranty_exp").datepicker(); });
...
<form name="frmasset" id="frmasset">
	...
	<table width=100% cellspacing=0 cellpadding=0 class="td_content" style="padding: 10px;">
		...
		<tr>
			...
			<td align=left  width=33% >
				<input type="text" name="acquired_date" id="acquired_date" class="nInput" style="width: 80%;" value="<?php echo $res['ad8']; ?>" >
			</td>
		...
		<tr><td height=4 colspan="4"></td></tr>
			...
			<td align=left  width=33%>
				<select name="dept_code" class="nInput" style="width:80%;" id="dept_code" >
					<?php
						$cc = $p->dbquery("select unitcode, costcenter from options_costcenter order by costcenter;");
						while($ccRow = $cc->fetch_array()) {
							echo "<option value='$ccRow[0]' ";
							if($res['dept_code'] == $ccRow[0]) { echo "selected"; }
							echo ">$ccRow[1]</option>";
						}
					?>
				</select>
		...
		<tr>
			<td colspan=4 style="padding-left: 40px;">
				<button type=button class="buttonding" stylye="height: 25px;" onclick="javascript: fa_save();" ><img src="images/icons/floppy.png" width=18 height=18 align=absmiddle />&nbsp;Save Changes Made</button>
				<?php if($_GET['fid'] != '') { ?>
