<?php
	...
	class _init extends myDB {
		...
		public function updateTStamp($skey) {
			parent::dbquery("update ignore active_sessions set `timestamp` = '".time()."' where sessid = '$skey';");
		}
		...
		function identCostCenter($id) {
			list($center) = parent::getArray("SELECT costcenter FROM options_costcenter a WHERE a.unitcode = '$id';");
			return $center;
		}
		...
		function identItemDescription($itemCode) {
			list($description) = parent::getArray("select description from products_master where item_code = '$itemCode';");
			return $description;
		}
		...
		function validateResult($table,$sono,$code,$serialno,$bid,$uid) {
			...
			parent::dbquery("update ignore lab_samples set validated_by = '$uid', validated_on = now() where so_no = '$sono' and serialno = '$serialno' and `code` = '$code';");
		...
		function updateLabSampleStatus($so,$code,$sn,$stat,$bid,$uid) {
			parent::dbquery("update ignore lab_samples set status = '$stat', updated_by = '$uid', updated_on = now() where so_no = '$so' and branch = '$bid' and code = '$code' and serialno = '$sn';");
		}
		...
		function deptOptions($uid,$utype,$all) {
			...
			if($utype == 'admin') {
				...
				$doQuery = parent::dbquery("SELECT id, dept_name FROM pccpayroll.options_dept ORDER BY dept_name asc;");
				while(list($did,$dname) = $doQuery->fetch_array()) {
					$deptOptions .= "<option value='$did'>$dname</option>";	
				}
				...
				switch($uid) {
					case "23": // Kix
					case "62": // Loraine
						$deptOptions = "<option value ='5'>LABORATORY</option>
										<option value = '7'>RADIOLOGY</option>
										<option value = '28'>PPP - PRIME CARE CEBU</option>
										<option value = '36'>PHARMACY</option>
										<option value = '39'>PCA (BASAK) - LABORATORY</option>
										<option value = '40'>PCA (BASAK) - RADIOLOGY</option>";
					break;
					case "6": // Pat
						$deptOptions = "<option value = '2'>HUMAN RESOURCE</option>";
					break;
					case "37": // Kenneth
					case "68": // Bea
						$deptOptions = "<option value = '11'>NURSING - CENTER</option>";
					break;

					case "18": // Switt
					case "28": // Racquel
					case "114": // Faye 
						$deptOptions = "<option value = '3'>MARKETING</option>";
					break;
					
					//case "91": // Emerlyn
					//	$deptOptions = "<option value = '28'>PPP - PRIME CARE CEBU</option>";
					//break;

					case "3": // Elmo
						$deptOptions = "<option value = '32'>IT DEPARTMENT</option>";
					break;

					case "11": // Janna
						$deptOptions = "<option value = '23'>FINANCE</option>
										<option value = '46'>PCA (BASAK) - ACCOUNTING</option>";
					break;

					case "4": // Janely
						$deptOptions = "<option value = '23'>TOTALMED</option>";
					break;

					case "26": // Arissa
					case "108": // Cieramae
						$deptOptions = "<option value = '9'>FRONTDESK</option>
										<option value = '44'>PCA (BASAK) - INFORMATION</option>";
					break;

					case "105": // Ace
					case "112": // Jellyn
					case "124": // Ma. Kristina
					case "33": // Michelle
						$deptOptions = "<option value = '8'>RELEASING</option>";
					break;

					case "110": // Paul Albino
					case "91": // Emerlyn
						$deptOptions = "
										<option value = '47'>NURSING - AIRPORT</option>
										<option value = '27'>NURSING - IPLOY</option>
										<option value = '24'>NURSING - KUEHNE NAGEL</option>
										<option value = '18'>NURSING - MARSH</option>
										<option value = '17'>NURSING - NOKIA</option>
										<option value = '31'>NURSING - NUSTAR</option>
										<option value = '14'>NURSING - TERADYNE</option>
										<option value = '16'>NURSING - WATERFRONT</option>
										<option value = '33'>NURSING - CCLEX</option>
										<option value = '34'>NURSING - AUTOLIV</option>
										<option value = '49'>DISTECH</option>
										<option value = '48'>NURSING - DIS TECH</option>
									";
					break;
				}
