<?php
	...
	$x = $con->getArray("select sum(debit-credit) from acctg_gl where cleared_on <= '$prev_date' and acct='$_POST[acct_code]' and cleared='Y' and branch = '1' group by acct;");
	
	$cbalance = $con->getArray("select ROUND(sum(debit-credit),2),abs(ROUND(sum(debit-credit),2)) from acctg_gl where tmp_cleared='Y' and acct='$_POST[acct_code]' and branch = '1';");
