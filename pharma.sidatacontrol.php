<?php
    ...
    switch($_REQUEST['mod']) {
        case "saveHeader":
            ...
            if($isCount > 0) {
                $queryString = "UPDATE IGNORE pharma_si_header set doc_date = '".$con->formatDate($_POST['doc_date'])."', si_no = '$_POST[si_no]', customer_code = '$_POST[cid]',customer_name = '".$con->escapeString(htmlentities($_POST['cname']))."',customer_address = '".$con->escapeString(htmlentities($_POST['caddr']))."',terms = '$_POST[terms]', payment_type= '$_POST[payment_type]', scpwd_id = '$_POST[sc_id]', physician = '".$con->escapeString(htmlentities($_POST['physician']))."', remarks = '".$con->escapeString(htmlentities($_POST['remarks']))."', updated_by = '$_SESSION[userid]' , updated_on = now() where doc_no = '$_POST[doc_no]';";
            } else {
                $queryString = "INSERT IGNORE INTO pharma_si_header (doc_no,branch,doc_date,si_no,customer_code,customer_name,customer_address,terms,payment_type,scpwd_id,physician,remarks,trace_no,created_by,created_on) VALUES ('$_POST[doc_no]','$bid','".$con->formatDate($_POST['doc_date'])."','$_POST[si_no]','$_POST[cid]','".$con->escapeString(htmlentities($_POST['cname']))."','".$con->escapeString(htmlentities($_POST['caddr']))."','$_POST[terms]','$_POST[payment_type]','$_POST[sc_id]','".$con->escapeString(htmlentities($_POST['physician']))."','".$con->escapeString(htmlentities($_POST['remarks']))."','$_POST[trace_no]','$_SESSION[userid]',now());";
            }
        ...
        case "updatePayment":
			$con->dbquery("update ignore pharma_si_header set balance = 0, amount_paid = ".$con->formatDigit($_POST['paid']).", cash_tendered = '".$con->formatDigit($_POST['cash'])."', cc_tendered = '".$con->formatDigit($_POST['cc'])."', change_due = '".$con->formatDigit($_POST['changeDue'])."', updated_by = '$uid', updated_on = now() where doc_no = '$_POST[doc_no]' and branch = '$bid';");
		break;
