<?php
	...
	switch($_POST['mod']) {
		...
		case "updateItem":
			...
			if($_POST['expiry'] != '') {
				$expiry = $p->formatDate($_POST['expiry']);
			} else { $expiry = '0000-00-00'; }

			$p->dbquery("UPDATE rr_details set costcenter = '$_POST[costcenter]', po_no = '$_POST[po]', po_date = '".$p->formatDate($_POST['podate'])."', item_code = '$_POST[item]',description='".$p->escapeString(htmlentities($_POST['description']))."',qty = '".$p->formatDigit($_POST['qty'])."',unit = '$_POST[unit]',cost = '".$p->formatDigit($_POST['cost'])."',amount = '".$p->formatDigit($_POST['amount'])."', lot_no = '$_POST[lotno]', expiry = '".$expiry."' where line_id = '$_POST[lid]';");
			$p->updateHeaderAmt($_POST['rr_no'],$bid);
		break;

		case "getPOS":
			list($b) = $p->getArray("select count(*) from (select a.po_no, date_format(a.po_date,'%m/%d/%Y') as pd8, a.remarks, ROUND(sum((b.cost-b.discount) * (b.qty-b.qty_dld)),2) as amount from po_header a left join po_details b on a.po_no=b.po_no and a.branch=b.branch where b.qty_dld < b.qty and a.supplier = trim(leading '0' from '$_POST[cid]') and a.status = 'Finalized' and a.po_no not in (select distinct po_no from rr_details where rr_no = '$_POST[rr_no]' and branch = '$bid') group by a.po_no) a;");
			if($b > 0) {
				...
				$c = $p->dbquery("select concat(lpad(a.branch,2,'0'),'-',lpad(a.po_no,6,0)) as po, a.po_no, a.po_date, a.branch, date_format(a.po_date,'%m/%d/%Y') as pd8, a.remarks, ROUND(sum((b.cost-b.discount) * (b.qty-b.qty_dld)),2) as amount from po_header a left join po_details b on a.po_no=b.po_no and a.branch=b.branch where b.qty_dld < b.qty and a.supplier = trim(leading '0' from '$_POST[cid]') and a.status = 'Finalized' and a.po_no not in (select distinct po_no from rr_details where rr_no = '$_POST[rr_no]' and branch = '$bid') group by a.po_no;");
		...
		case "retrieve":
			...
			$srrd = $p->dbquery("SELECT line_id AS id, a.costcenter, b.costcenter AS cc, LPAD(po_no,6,0) AS po, IF(po_date!='0000-00-00',DATE_FORMAT(po_date,'%m/%d/%Y'),'') AS podate, description, item_code, unit, qty, cost, amount, lot_no, if(expiry!='0000-00-00',date_format(expiry,'%m/%d/%Y'),'') as expiry FROM rr_details a LEFT JOIN options_costcenter b ON a.costcenter = b.unitcode where trace_no = '$_REQUEST[trace_no]';");
