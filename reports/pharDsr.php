<?php
	...
	/* MYSQL QUERIES SECTION */
		...
		$query = $mydb->dbquery("SELECT * FROM (SELECT 'so' AS `type`, a.so_no, DATE_FORMAT(b.so_date, '%m/%d/%Y') AS so_date, '' AS si_no, '' AS sidate, b.customer_code, b.customer_name, b.patient_name, c.description AS xterms, a.code, a.description, a.unit_price, a.qty, a.amount, a.discount, ROUND(a.amount-a.discount,2) AS charge_sales, '0' AS cash_sales, '0' AS cc_sales FROM pharma_so_details a LEFT JOIN pharma_so_header b ON a.trace_no = b.trace_no LEFT JOIN options_terms c ON b.terms = c.terms_id WHERE b.status = 'Finalized' AND b.terms NOT IN ('0','100') AND b.so_date BETWEEN '$dtf' AND '$dt2' UNION ALL SELECT 'si' AS `type`, a.so_no, DATE_FORMAT(b.doc_date,'%m/%d/%Y') AS so_date, b.si_no AS si_no, DATE_FORMAT(b.doc_date,'%m/%d/%Y') AS sidate, b.customer_code, b.customer_name, b.patient_name, 'CASH' AS terms, a.code, a.description, a.unit_price, a.qty,  a.amount AS amount, a.discount, '0' AS charge_sales, ROUND(a.amount-a.discount,2) AS cash_sales, '0' AS cc_sales FROM pharma_si_details a LEFT JOIN pharma_si_header b ON a.trace_no = b.trace_no WHERE b.status= 'Finalized' AND b.doc_date BETWEEN '$dtf' AND '$dt2' AND b.cash_tendered != '0' UNION ALL SELECT 'cc' AS `type`, a.so_no, DATE_FORMAT(b.doc_date,'%m/%d/%Y') AS so_date, b.si_no AS si_no, DATE_FORMAT(b.doc_date,'%m/%d/%Y') AS sidate, b.customer_code, b.customer_name, b.patient_name, 'CASH' AS terms, a.code, a.description, a.unit_price, a.qty,  a.amount AS amount, a.discount, '0' AS charge_sales, '0' AS cash_sales, ROUND(a.amount-a.discount,2) AS cc_sales FROM pharma_si_details a LEFT JOIN pharma_si_header b ON a.trace_no = b.trace_no WHERE b.status= 'Finalized' AND b.doc_date BETWEEN '$dtf' AND '$dt2' AND b.payment_type = 'Card') a ORDER BY so_no, si_no;");
...
$cashGT = 0; $chargeGT = 0; $cardGT = 0; $i = 1;
while($row = $query->fetch_array()) {
	...
	switch($row['type']) {
		...
		case "cc":
			if($row['si_no'] != $xso) {
				$card = number_format($row['cc_sales'],2); $charge = ''; $cash = ''; $cardGT += $row['cc_sales'];
			}
		break;
	}
