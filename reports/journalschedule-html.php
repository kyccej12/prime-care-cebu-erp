<?php
	...
	switch($_GET['type']) { 
		case "CR": $lbl = "Collection Receipts Journal"; $f1 = "and a.doc_type = 'OR' "; break;
	...
	/* MYSQL QUERIES SECTION */
		...
		$query = $mydb->dbquery("SELECT doc_no, CONCAT(LPAD(branch,2,0),'-',LPAD(doc_no,5,0)) AS dno, DATE_FORMAT(doc_date,'%m/%d/%Y') AS dd8, a.acct, b.description, ROUND(sum(debit),2) as debit, ROUND(sum(credit),2) as credit, c.tradename, contact_id, CONCAT(a.branch,'-',doc_no) AS xdoc, doc_remarks FROM acctg_gl a LEFT JOIN acctg_accounts b ON a.acct=b.acct_code LEFT JOIN contact_info c ON a.contact_id=c.file_id WHERE doc_date BETWEEN '".$mydb->formatDate($_GET['dtf'])."' AND '".$mydb->formatDate($_GET['dt2'])."' $f1 $f2 $f3 group by a.doc_no, a.acct order by doc_no asc;");
	/* END OF SQL QUERIES */
