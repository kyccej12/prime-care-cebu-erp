<?php
	...
	/* MYSQL QUERIES SECTION */
		...
		$query = $mydb->dbquery("SELECT * FROM (SELECT 'so' AS `type`, a.so_no, DATE_FORMAT(a.so_date,'%m/%d/%Y') AS sodate,'' AS or_no, '' AS ordate, a.customer_code, a.customer_name, a.patient_id, a.patient_name, c.description AS xterms, b.code,b.description, b.amount_due, a.amount AS charge_sales, '0' AS cash_sales, '0' AS cc_sales, '0' AS check_sales, '0' AS ad_payment, a.referred_by FROM so_header a LEFT JOIN so_details b ON a.trace_no = b.trace_no LEFT JOIN options_terms c ON a.terms = c.terms_id LEFT JOIN services_master d ON b.code = d.code WHERE a.status = 'Finalized' AND a.terms NOT IN ('0','100') AND a.so_date BETWEEN '$dtf' AND '$dt2' $searchString UNION ALL SELECT 'ap' AS `type`, a.so_no, DATE_FORMAT(a.so_date,'%m/%d/%Y') AS sodate,'' AS or_no, '' AS ordate, a.customer_code, a.customer_name, a.patient_id, a.patient_name, c.description AS xterms, b.code,b.description, b.amount_due, '0' AS charge_sales, '0' AS cash_sales, '0' AS cc_sales, '0' AS check_sales, a.amount AS ad_payment, a.referred_by FROM so_header a LEFT JOIN so_details b ON a.trace_no = b.trace_no LEFT JOIN options_terms c ON a.terms = c.terms_id WHERE a.status = 'Finalized' AND a.terms IN ('100') AND a.so_date BETWEEN '$dtf' AND '$dt2' $searchString UNION ALL SELECT 'or' AS `type`, b.so_no, DATE_FORMAT(b.so_date,'%m/%d/%Y') AS so_date, a.or_no AS or_no, DATE_FORMAT(a.doc_date,'%m/%d/%Y') AS ordate, a.customer_code, a.customer_name, b.pid AS patient_id, b.pname AS patient_name, 'Cash' AS terms, b.code, b.description, b.amount_due, 0 AS charge_sales, a.amount_due AS cash_sales, '0' AS cc_sales, '0' AS check_sales, '0' AS ad_payment, '' as referred_by FROM or_header a LEFT JOIN or_details b ON a.trace_no = b.trace_no WHERE a.status = 'Finalized' AND a.doc_date BETWEEN '$dtf' AND '$dt2' AND a.cashtype IN ('1') $searchString UNION ALL SELECT 'cc' AS `type`, b.so_no, DATE_FORMAT(b.so_date,'%m/%d/%Y') AS so_date, a.doc_no AS or_no, DATE_FORMAT(a.doc_date,'%m/%d/%Y') AS ordate, a.customer_code, a.customer_name, b.pid AS patient_id, b.pname AS patient_name, 'Cash' AS terms, b.code, b.description, a.amount_due, 0 AS charge_sales, cash_tendered AS cash_sales, cc_tendered AS cc_sales, '0' AS check_sales, '0' AS ad_payment, '' as referred_by FROM or_header a LEFT JOIN or_details b ON a.doc_no = b.doc_no WHERE a.status = 'Finalized' AND a.doc_date BETWEEN '$dtf' AND '$dt2' AND a.cashtype IN ('2') $searchString UNION ALL SELECT 'ck' AS `type`, b.so_no, DATE_FORMAT(b.so_date,'%m/%d/%Y') AS so_date, a.doc_no AS or_no, DATE_FORMAT(a.doc_date,'%m/%d/%Y') AS ordate, a.customer_code, a.customer_name, b.pid AS patient_id, b.pname AS patient_name, 'Cash' AS terms, b.code, b.description, b.amount_due, 0 AS charge_sales, '0' AS cash_sales, '0' AS cc_sales, a.amount_due AS check_sales, '0' AS ad_payment, '' as referred_by FROM or_header a LEFT JOIN or_details b ON a.doc_no = b.doc_no WHERE a.status = 'Finalized' AND a.doc_date BETWEEN '$dtf' AND '$dt2' AND a.cashtype IN ('3') $searchString) a ORDER BY or_no, so_no;");
...
<table class="items" width="100%" align=center style="font-size: 7pt; border-collapse: collapse;" cellpadding="3">
	<thead>
		<tr>
			...
			<td align=center><b>REFERRED BY</b></td>
...
while($row = $query->fetch_array()) {
	...
	list($employer) = $mydb->getArray("select employer from patient_info where patient_id = '$row[patient_id]';");
		...
		switch($row['type']) {
			...
			case "cc":	if($row['or_no'] != $xor) {	$cc = $row['cc_sales']; $charge = ''; $cash = $row['cash_sales']; $ap = ''; $ccGT += $row['cc_sales']; $cashGT+=$row['cash_sales']; }	break;
	...
	$html = $html . '<tr bgcolor="'.$mydb->initBackground($i).'">
		...
		<td align=left width=12%>' . $billedto . '</td>
		<td align=left width=12%>'. $row['patient_name'] .'</td>
		<td align=left>'.  $employer . '</td>
		<td align=center>'. $row['referred_by'] .'</td>
		<td align=center>'. $row['xterms'] .'</td>
		<td align=left width=12%>'. $row['description'] .'</td>
