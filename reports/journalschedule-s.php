<?php
	...
	switch($_GET['type']) { 
		case "CR": $lbl = "Collection Receipts Journal Summary"; $f1 = "and a.doc_type = 'OR' "; break;
	...
	/* MYSQL QUERIES SECTION */
		...
		$query = $mydb->dbquery("select a.acct, b.description, ROUND(sum(debit-credit),2) as amt from acctg_gl a left join acctg_accounts b on a.acct=b.acct_code and a.company=b.company where doc_date between '".$mydb->formatDate($_GET['dtf'])."' and '".$mydb->formatDate($_GET['dt2'])."' and a.company='$_SESSION[company]' $f1 $f2 $f3 group by a.acct;");
	/* END OF SQL QUERIES */
