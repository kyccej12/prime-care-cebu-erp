<?php
	...
	/* MYSQL QUERIES SECTION */
		...
		$query = $mydb->dbquery("SELECT if(a.cost_center!='',concat(a.acct,'-',a.cost_center),a.acct) as acct, a.cost_center, b.description, SUM(debit-credit) AS amt FROM acctg_gl a LEFT JOIN acctg_accounts b ON a.acct=b.acct_code WHERE doc_date <= '".$mydb->formatDate($_GET['asof'])."' GROUP BY a.acct,a.cost_center order by a.acct;");
	/* END OF SQL QUERIES */
...
<thead>
	<tr>
		<td width="15%" align=left><b>ACCOUNT CODE</b></td>
		<td width="20%" align=left><b>COST CENTER</b></td>
		<td width="35%" align=left><b>ACCOUNT DESCRIPTION</b></td>
...
while($row = $query->fetch_array(MYSQLI_BOTH)) {
	if($row['amt'] != 0) {
		...
		list($costcenter) = $mydb->getArray("select costcenter from options_costcenter where unitcode = '$row[cost_center]';");
