<?php
	...
	if($_GET['type'] == 'Exp') { 
		...
		$outQuery = $db->dbquery("select acct_code as acct,description from acctg_accounts where acct_grp in ($acctGroup) and parent != 'Y' order by acct_code;");
	} else { 
		$lbl = "Revenue"; 
		$subLbl = "Target"; 
		$val = "sum(credit-debit) as amt"; 

		$outQuery = $db->dbquery("select sales_id as acct,sales_category as description from options_salescategory  order by sales_category;");
	}
